...
Every module now uses executable `examples/` and a classified `unit.tftest.hcl` or `integration.tftest.hcl` test name.

All 44 modules are normalized reference implementations.

Use [MODULES_INDEX.md](modules-index.md) when you want the full module-by-module link map.
...
- Module test files stay outside the default CI path. Mocked tests use `unit.tftest.hcl` and real-provider tests use `integration.tftest.hcl`; review every integration provider and run block before execution.
