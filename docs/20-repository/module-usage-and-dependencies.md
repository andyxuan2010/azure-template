...
5. Use the module `examples/` folder as the starting point.
...
8. Check the test filename and provider declarations before running `terraform test`. `unit.tftest.hcl` uses mocks; `integration.tftest.hcl` uses real providers and requires an impact review.
...
| `eventgridsystemtopic` | Event Grid System Topic for an existing Azure event source | Existing resource group; existing Event Grid-compatible source resource | Event subscription, delivery endpoint, dead-letter storage, managed identity, downstream RBAC | `modules/eventgridsystemtopic/examples/` |
...
| `route_table` | User-defined routes and subnet associations | Existing resource group | VNet subnets, Azure Firewall or network virtual appliance | `modules/route_table/examples/` |
| `servicebus` | Service Bus namespace with queues, topics, and subscriptions | Existing resource group | Private endpoint subnet, private DNS zone, network rules, Log Analytics workspace, control- and data-plane RBAC | `modules/servicebus/examples/` |
| `subscription_vending` | Subscription alias creation or existing-subscription bootstrap | Existing subscription context, or a billing scope for alias-only stage one | Management group, provider registrations, bootstrap resource groups | `modules/subscription_vending/examples/` |
...
| `functionappflex` | Flex Consumption Function App | Existing resource group; Linux FC1 App Service Plan; blob deployment container | VNet integration subnet, package artifact, storage identity, Application Insights, downstream RBAC | `modules/functionappflex/examples/` |
...
| `sqldb` | Azure SQL logical server and single database | Existing resource group; Entra administrator; SQL credentials unless Entra-only | Private endpoint subnet and DNS, diagnostics, Key Vault keys and identities, failover partner | `modules/sqldb/examples/` |
| `sqlmi` | Azure SQL Managed Instance | Existing resource group; dedicated delegated subnet; SQL administrator credential | Diagnostics workspace, DNS-zone partner instance, user-assigned identities, Entra administrator | `modules/sqlmi/examples/` |
| `sqlmi_db` | Database on an existing SQL Managed Instance | Existing SQL Managed Instance | Diagnostics workspace, control-plane RBAC groups | `modules/sqlmi_db/examples/` |
| `sqlvm` | SQL Server on Azure Windows VMs with SQL IaaS registration | Existing application resource group and database subnet; administrator credentials | Availability Set or zones, domain services, SQL VM group/listener composition | `modules/sqlvm/examples/` |
| `storageaccount` | Secure Storage account, data services, private connectivity, diagnostics, and RBAC | Existing resource group; default and `azurerm.prod` provider configurations | Subnet rules, private endpoint subnet and DNS zones, Key Vault key/identity, Log Analytics | `modules/storageaccount/examples/` |
| `staticwebapp` | Azure Static Web App with Free-plan defaults and optional deployment integration | Existing resource group | Repository token, managed identity, custom domain and DNS, monitoring, deployment workflow | `modules/staticwebapp/examples/` |
| `subnet` | Subnets in an existing VNet with delegation and optional associations | Existing VNet and address plan | NSGs, route tables, service endpoint policies, VNet-scoped RBAC | `modules/subnet/examples/` |
| `vnet` | Virtual network and optional initial subnets | Existing resource group and address plan | Custom DNS, DDoS protection plan, VNet-scoped RBAC, Log Analytics | `modules/vnet/examples/` |
| `winvm` | Windows VMs built against the shared IaC foundation | Existing application/shared-IaC resource groups, VNet/subnet, Key Vault, Storage account | Domain services, ADF SHIR, access groups, Log Analytics | `modules/winvm/examples/` |
...
- `rg` -> `eventhub`
- `rg` -> existing event source -> `eventgridsystemtopic` -> event subscription
- `rg` -> `openai`
...
- `rg` -> `appserviceplan` with FC1 + blob deployment container -> `functionappflex`
...
Use this module to connect an existing Azure resource to Event Grid's system-topic model. Create the source resource first, pass its output ID and matching topic type, then create event subscriptions separately with explicit delivery, filtering, retry, dead-letter, identity, and RBAC ownership.
...
Use this module to control subnet routing explicitly. The normal landing zone pattern is `vnet -> firewall -> route_table -> workload subnets`. Keep association ownership in one state, validate forward and return paths, and stage any default-route change because it can immediately interrupt every attached subnet.

### `servicebus`

Use this module when the workload needs queues, topics, and subscriptions in a shared messaging namespace. Feed Private Endpoint, DNS, monitoring, and network inputs from the platform foundation. Explicitly disable SAS and public access for hardened environments, and create Service Bus data-plane roles separately because the module's group assignments are Azure control-plane roles.
...
New-subscription mode must be staged: create the alias first, then configure a provider for the returned subscription and run bootstrap from a second root/state. Provider configuration cannot be derived dynamically from a resource created in the same apply.
...
Use this separate module when the workload targets Azure Functions Flex Consumption. Create a Linux `FC1` plan and blob deployment container first, then pass the runtime name/version and storage configuration. Flex-specific memory, concurrency, maximum-instance, and always-ready settings are workload decisions; this module does not create the plan, package artifact, or downstream RBAC.
...
Use this module when one Azure SQL logical server and database should be managed together. Prefer Entra-only authentication, direct Private Endpoint and DNS IDs, auditing, diagnostics, and reviewed retention. Protect state whenever credentials, Key Vault secret lookups, import keys, or audit keys are used, and prepare identities and Key Vault permissions before enabling customer-managed encryption.

### `sqlmi`

Use this module when a workload needs SQL Managed Instance rather than a single Azure SQL Database. Provision and validate the dedicated delegated subnet before the instance, confirm quota and cost approval, and plan for multi-hour create, scale, and delete operations. SQL users, databases, backup recovery, and failover remain separate operational concerns.

### `sqlmi_db`

Use this module to add one database to an already-provisioned SQL Managed Instance. Feed the parent name and resource group from `sqlmi` outputs, configure diagnostics where required, and manage SQL users separately. Its group assignments are control-plane roles; deleting the module deletes the managed database, so confirm recovery controls first.
...
Use this module for storage consumed by applications, VM bootstrapping, diagnostics, or private workloads. Its default posture disables public access and denies unmatched network traffic, so establish an approved private or firewall path before data-plane provisioning. Pass `azurerm.prod` for shared private DNS lookup and disable broad automatic execution-identity grants when RBAC is managed centrally.

### `staticwebapp`

Use this module for static web workloads and frontend/API deployments that fit Azure Static Web Apps. It defaults to the Free SKU, serves public content by default, and does not create an App Service Plan. Supply the repository integration values only through protected secret handling, or use the Static Web App deployment API key output in a reviewed CI/CD workflow.

### `subnet`

Use this module when subnets need an independent lifecycle inside an existing VNet. Pass the VNet ID from upstream outputs, set explicit association flags with NSG/route IDs, and keep each association owned by one Terraform state. Treat subnet key, address, delegation, and policy changes as migration-sensitive once workloads are attached.

### `vnet`

Use this module to build the network foundation for other modules. Confirm the address plan before creation, define only initial subnets that share the VNet lifecycle, and hand output IDs to downstream modules. Use the separate `subnet` module when subnet ownership must evolve independently.

### `winvm`

Use this module for private Windows VM patterns with optional bootstrap, SHIR, or domain join integration. Create the network and shared IaC Key Vault/Storage dependencies first. Microsoft Entra login and domain join are mutually exclusive, Run Command is the preferred bootstrap path, and the built-in VM identity Key Vault roles require a least-privilege review.

## Test Coverage Notes

- All modules use `unit.tftest.hcl` or `integration.tftest.hcl` according to provider behavior.
...
- Forty-two module test files use mock providers and plan-only or expected-failure runs; they do not create cloud or tenant resources.
...
- The `acr` and `adf` integration tests use real providers and must be reviewed for authentication, network access, cost, and cleanup before execution.
