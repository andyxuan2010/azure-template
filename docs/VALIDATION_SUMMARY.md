...
- `modules/fortigate` is included in the root harness and changed-module CI matrix, with a mocked plan test covering single, active-passive, public-frontend opt-in, and module-created subnet scenarios.
...
- `modules/loadbalancer` is included in the root harness and changed-module CI matrix, with root `loadbalancers` map support for one or more Load Balancer module instances.
