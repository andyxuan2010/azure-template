...
- `README.md`
- executable scenarios under `examples/`
- `tests/unit.tftest.hcl` and/or `tests/integration.tftest.hcl`
- root `variables.tf`
- root `main.tf`
- root `terraform.tfvars`

Modules awaiting documentation normalization may still use the legacy `EXAMPLES.md`, `QUICK_REFERENCE.md`, `INDEX.md`, and `tests/live.tftest.hcl` names.
...
- supports direct subnet and Private DNS zone IDs
- supports subnet and Private DNS zone lookup by name
- uses `azurerm.prod` for shared subnet and DNS lookups
- uses the default `azurerm` provider to create the endpoint

Primary files:

- `modules/private_endpoint/data.tf`
- `modules/private_endpoint/locals.tf`
- `modules/private_endpoint/terraform.tf`
- `modules/private_endpoint/examples/shared-network-lookups/`

The root module and every example pass both provider configurations explicitly.
