...
- Azure DevOps mirrors the same pattern in `azure-pipelines.yml`.
- The matrix jobs use `terraform init -backend=false -reconfigure` so the harness can validate module wiring without competing for shared state.
