...
- `modules/loadbalancer` exists as a standalone module but is not currently wired into the root harness or CI module matrix
...
34. `sqlmi_db`
35. `storageaccount`
36. `subscription_vending`
37. `vnet`
38. `winvm`

`loadbalancer` is intentionally omitted from this list until a root module block, feature flag, root variables, and CI matrix entry are added for it.
