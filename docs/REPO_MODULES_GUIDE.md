...
3. Enable only the target entry in `module_plan_enabled`.
4. Run `terraform init -backend=false -reconfigure` when you want a non-stateful harness check.
5. Run `terraform plan` from the repo root.
