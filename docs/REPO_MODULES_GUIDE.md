...
- Tagging: ARM-tagged modules should follow [TAGGING_STANDARD.md](./TAGGING_STANDARD.md), merging inherited tags, `var.tags`, and mandatory `Environment`/`Workload` tags with mandatory tags last.
...
- Add `locals.tf` with the standard tag locals from [TAGGING_STANDARD.md](./TAGGING_STANDARD.md)
- Use the module's final tag local on all ARM resources that support tags
