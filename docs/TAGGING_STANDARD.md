...
Some Azure resource types may still show recurring tag-only drift when policy remediation or platform behavior mutates tags after creation. In those cases, keep the module inheritance inputs enabled and use a narrow `lifecycle.ignore_changes = [tags]` on the affected resource only. The private DNS zone module follows this pattern for private DNS zones and virtual network links so policy-managed tag inheritance does not create repeated no-op updates.
