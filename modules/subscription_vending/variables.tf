...
variable "name" {
  description = "Optional display name override for the subscription. Leave empty to generate one from the global naming convention."
  type        = string
  default     = ""

  validation {
    condition     = trimspace(var.name) == "" || length(trimspace(var.name)) <= 64
    error_message = "name must be empty or 64 characters or less."
  }
}

variable "subscription_name" {
  description = "Deprecated alias for name. Use name for the subscription display name override."
  type        = string
  default     = ""

  validation {
    condition     = trimspace(var.subscription_name) == "" || length(trimspace(var.subscription_name)) <= 64
    error_message = "subscription_name must be empty or 64 characters or less."
  }
}
...
variable "billing_scope_id" {
  ...
  validation {
    condition     = trimspace(var.billing_scope_id) == "" || startswith(trimspace(var.billing_scope_id), "/providers/Microsoft.Billing/")
    error_message = "billing_scope_id must be empty or a Microsoft Billing resource ID."
  }
}
...
variable "resource_provider_registrations" {
  ...
  validation {
    condition     = alltrue([for value in var.resource_provider_registrations : can(regex("^Microsoft\\.[A-Za-z0-9.]+$", trimspace(value)))])
    error_message = "resource_provider_registrations must contain valid Microsoft resource provider namespaces."
  }
}
...
variable "bootstrap_resource_groups" {
  ...
  validation {
    condition = alltrue([
      for _, resource_group in var.bootstrap_resource_groups :
      trimspace(resource_group.name) != "" &&
      trimspace(resource_group.location) != "" &&
      alltrue([for k, v in resource_group.tags : trimspace(k) != "" && trimspace(v) != ""])
    ])
    error_message = "Each bootstrap resource group must have a non-empty name and location, with non-empty tag keys and values."
  }
}
...
check "subscription_vending_consistency" {
  ...
  assert {
    condition     = !var.enable_management_group_association || trimspace(var.management_group_id) != ""
    error_message = "management_group_id must be set when enable_management_group_association is true."
  }
}
