...
Provision an Azure Automation Account with secure defaults, standardized naming and tags, managed identity, RBAC, private endpoints, diagnostics, runbooks, schedules, and Automation variables.

## Features

- Secure baseline with local authentication disabled, public network access disabled, and system-assigned identity enabled by default.
- Standard generated naming using `name_prefix`, `workload_name`, `app_env`, `location_code`, and optional random suffixes.
- Compact standard tags for Automation Account's 15-tag limit, including `app_env` and environment-specific tags.
- System-assigned and user-assigned managed identity support, including optional customer-managed key encryption.
- Built-in Contributor and Reader assignments for Entra groups, plus generic Automation Account scope RBAC assignments.
- Optional role assignments for the Automation Account system-assigned managed identity.
- Optional Webhook and DSC/Hybrid Worker private endpoints with legacy input compatibility.
- Diagnostics to Log Analytics, Storage Account archive, and Event Hub.
- Optional runbooks, schedules, job schedules, and string, bool, int, datetime, and object variables.
...
module "automationaccount" {
  ...
  private_dns_zone_id             = "/subscriptions/<sub>/resourceGroups/<rg>/providers/Microsoft.Network/privateDnsZones/privatelink.azure-automation.net"
...
You can also look up the private DNS zone by name:

```hcl
private_dns_zone_name                = "privatelink.azure-automation.net"
private_dns_zone_resource_group_name = "rg-private-dns"
```
  ...
  runbooks = {
    cleanup = {
      ...
      publish_content_link = {
        uri = "https://raw.githubusercontent.com/example/org/main/runbooks/cleanup.ps1"
      }
    }
  }
...
- Use `private_endpoint_subnet_id` when possible. Legacy VNet/subnet lookup inputs are kept for compatibility.
- Do not store secrets in Automation variables through Terraform unless your state backend is protected appropriately. Encrypted Automation variables are still represented in Terraform state.
- Set `use_random_suffix = false` only when deterministic generated names are required.
