...
run "plan_named_automation_account" {
  ...
  assert {
    condition     = output.tags.module == "automationaccount" && output.tags.ManagedBy == "Terraform" && output.tags.Environment == "Production"
    error_message = "Effective tags did not include standardized Automation Account tags."
  }
}
...
run "plan_generated_name_without_random" {
  ...
  assert {
    condition     = output.name == "aa-shared-poc-eus-001"
...
run "plan_private_endpoint_runbooks_schedules_and_variables" {
  ...
  variables {
    ...
    private_dns_zone_id             = "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/rg-ba-eus-prd-shared-management/providers/Microsoft.Network/privateDnsZones/privatelink.azure-automation.net"
    ...
    job_schedules = {
      hello_daily = {
        ...
        schedule_name = "daily"
        parameters = {
          resourcegroup = "rg-ba-eus-prd-shared-management"
        }
      }
    }
