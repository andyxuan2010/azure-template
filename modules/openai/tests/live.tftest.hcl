...
run "plan_secure_defaults" {
  ...
  assert {
    condition     = output.merged_tags.module == "openai" && output.merged_tags.ManagedBy == "Terraform" && output.merged_tags.Environment == "Development"
    error_message = "Azure OpenAI merged tags did not include standardized tags."
  }
}
...
run "plan_generated_name_without_random" {
  ...
  assert {
    condition     = output.name == "oai-platform-poc-eus-001"
    error_message = "Generated Azure OpenAI name did not match the expected naming convention."
  }

  assert {
    condition     = output.custom_subdomain_name == "oai-platform-poc-eus-001"
