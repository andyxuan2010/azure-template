...
run "plan_secure_defaults" {
  ...
  assert {
    ...
    error_message = "Cosmos DB tag inheritance or explicit-tag normalization failed."
  }
}
...
run "plan_generated_name_without_random" {
  ...
  assert {
    condition     = output.name == "cosmos-orders-poc-cc-001"
...
run "plan_legacy_dns_zone_and_bounded_staleness" {
  command = plan

  variables {
    resource_group_name = "rg-platform-prod"
    location            = "canadacentral"
    name                = "cosmos-iactest-prod-002"
  ...
  assert {
    condition     = contains(output.private_dns_zone_ids, var.private_dns_zone_id)
    error_message = "The backward-compatible private_dns_zone_id input must be included in the private endpoint DNS zone group."
  }
}
