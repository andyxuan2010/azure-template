...
variable "geo_locations" {
  ...
  validation {
    condition = (
      length(distinct([for item in var.geo_locations : lower(trimspace(item.location))])) == length(var.geo_locations) &&
      length(distinct([for item in var.geo_locations : item.failover_priority])) == length(var.geo_locations) &&
      alltrue([for item in var.geo_locations : trimspace(item.location) != "" && item.failover_priority >= 0])
    )
    error_message = "geo_locations must contain unique non-empty locations and unique non-negative failover priorities."
  }
}
...
variable "consistency_policy" {
  ...
  validation {
    condition = var.consistency_policy.consistency_level == "BoundedStaleness" ? (
      try(var.consistency_policy.max_interval_in_seconds, null) != null &&
      try(var.consistency_policy.max_staleness_prefix, null) != null &&
      var.consistency_policy.max_interval_in_seconds >= 5 &&
      var.consistency_policy.max_interval_in_seconds <= 86400 &&
      var.consistency_policy.max_staleness_prefix >= 10 &&
      var.consistency_policy.max_staleness_prefix <= 2147483647
      ) : (
      try(var.consistency_policy.max_interval_in_seconds, null) == null &&
      try(var.consistency_policy.max_staleness_prefix, null) == null
    )
    error_message = "BoundedStaleness requires max_interval_in_seconds (5-86400) and max_staleness_prefix (10-2147483647); other consistency levels must omit them."
  }
}
...
variable "total_throughput_limit" {
  ...
  validation {
    condition     = var.total_throughput_limit == null ? true : (var.total_throughput_limit == -1 || var.total_throughput_limit >= 1000)
    error_message = "total_throughput_limit must be null, -1, or at least 1000 RU/s."
  }
}
...
variable "key_vault_key_id" {
  ...
  validation {
    condition     = trimspace(var.key_vault_key_id) == "" || can(regex("^https://.+\\.(vault\\.azure\\.net|managedhsm\\.azure\\.net)/keys/.+$", var.key_vault_key_id))
    error_message = "key_vault_key_id must be empty or a valid Key Vault or Managed HSM key URI."
  }
}
...
variable "private_dns_zone_ids" {
  ...
  validation {
    ...
    error_message = "private_dns_zone_ids must contain valid Azure Private DNS Zone resource IDs."
  }
}
...
check "cosmosdb_input_consistency" {
  ...
  assert {
    condition = !contains(var.capabilities, "EnableServerless") || (
      length(local.geo_locations_effective) == 1 &&
      alltrue([for database in values(var.sql_databases) : try(database.throughput, null) == null && try(database.autoscale_max_ru, null) == null]) &&
      alltrue([for container in values(var.sql_containers) : try(container.throughput, null) == null && try(container.autoscale_max_ru, null) == null])
    )
    error_message = "Serverless accounts support one geo location and cannot configure database or container throughput."
  }
  assert {
    condition = alltrue([
  ...
  assert {
    condition = alltrue([
      ...
      contains(keys(var.sql_databases), container.database_name)
    ])
    error_message = "Every sql_containers.database_name must reference a key in sql_databases."
  }

  assert {
    condition = (
      length(local.private_dns_zone_names_effective) == 0 ||
      trimspace(var.private_dns_zone_resource_group_name) != ""
    )
    error_message = "private_dns_zone_resource_group_name is required when private DNS zone names are supplied."
  }
  ...
  assert {
    condition     = trimspace(var.key_vault_key_id) == "" || local.identity_enabled
    error_message = "key_vault_key_id requires a system-assigned or user-assigned managed identity."
  }
