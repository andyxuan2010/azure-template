locals {
  ...
  resolved_location = trimspace(var.location) != "" ? trimspace(var.location) : data.azurerm_resource_group.this[0].location
  ...
  generated_name    = trimsuffix(substr("${var.name_prefix}-${local.workload_code}-${local.location_code}-${var.app_env}-${trimspace(var.instance)}", 0, 50), "-")

  eventgrid_system_topic_name = trimspace(var.name) != "" ? trimspace(var.name) : local.generated_name
