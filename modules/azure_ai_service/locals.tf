locals {
  ...
  account_name = trimspace(var.name) != "" ? trimspace(var.name) : "ais${random_string.random[0].result}"
  custom_subdomain_name = trimspace(var.custom_subdomain_name) != "" ? trimspace(var.custom_subdomain_name) : (
    var.enable_private_endpoint ? local.account_name : null
  )
