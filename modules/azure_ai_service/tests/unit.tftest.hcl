...
run "plan_named_ai_service" {
  ...
  assert {
    ...
    error_message = "Azure AI Services should preserve caller tags without adding module-generated tags."
  }
}

run "reject_invalid_network_acl_ip" {
  command = plan

  variables {
    network_acls = {
      default_action = "Deny"
      ip_rules       = ["999.999.999.999"]
    }
  }
...
run "reject_network_injection_without_identity" {
  command = plan
