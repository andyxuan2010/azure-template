...
run "plan_named_ai_service" {
  ...
  assert {
    condition     = output.tags.module == "azure_ai_service" && output.tags.ManagedBy == "Terraform" && output.tags.Environment == "Production"
...
run "plan_generated_name_without_random" {
  ...
  assert {
    condition     = output.name == "ais-shared-poc-eus-001"
