...
Provision an Azure AI Services account with secure defaults, standardized naming and tags, managed identity, network controls, private endpoints, diagnostics, RBAC, Responsible AI policies, and optional model deployments.

## Features

- Secure baseline with public network access and local key authentication disabled by default.
...
- `AIServices` by default, with optional `kind` override for other Cognitive Services account kinds.
- System-assigned and user-assigned managed identity support, plus legacy `identity` input compatibility.
- Customer-managed key, storage attachment, network ACL, outbound FQDN, Metrics Advisor, QnA, and Custom Question Answering options.
- Optional AI Foundry project management and network injection.
- Built-in Contributor and Reader assignments for Entra groups, plus generic account-scope RBAC assignments.
- Optional private endpoint with DNS zone IDs or DNS zone name lookup.
- Diagnostics to Log Analytics, Storage Account archive, and Event Hub.
- Optional Responsible AI policies and Cognitive deployments.
...
- `project_management_enabled` and `network_injection` require `kind = "AIServices"` and a managed identity.
- Model deployments and Responsible AI policies require `kind = "AIServices"` or `kind = "OpenAI"`.
- Network ACL IP rules accept IPv4 addresses and CIDR ranges; IPv6 is not supported by the provider resource.
- Private endpoints, network ACLs, project management, network injection, and Custom Question Answering require a custom subdomain; the module auto-generates one from the account name when needed.
- `customer_managed_key.key_vault_key_id` must be the Key Vault key identifier URI, for example `https://vault.vault.azure.net/keys/key/version`.
