...
run "plan_named_databricks_workspace" {
  ...
  assert {
    ...
    error_message = "Databricks tag inheritance or explicit-tag normalization failed."
  }
}
...
run "plan_vnet_injection_rbac_and_diagnostics" {
  ...
  variables {
    ...
    custom_parameters = {
      ...
      no_public_ip                                         = true
      storage_account_name                                 = "stdbxiactestprod001"
  ...
  assert {
    condition     = output.role_assignment_count == 7
    error_message = "Role assignment count should include workspace, custom storage account, and custom assignments."
  }
}
