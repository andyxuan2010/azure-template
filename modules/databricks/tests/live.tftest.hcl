...
run "plan_named_databricks_workspace" {
  ...
  assert {
    condition     = output.tags.module == "databricks" && output.tags.ManagedBy == "Terraform" && output.tags.Environment == "Production"
...
run "plan_generated_name_without_random" {
  ...
  assert {
    condition     = output.name == "dbw-shared-poc-eus-001"
