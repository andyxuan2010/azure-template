...
- Optional default storage account RBAC when `custom_parameters.storage_account_name` is set: `app_admin_group` receives Contributor and Storage Blob Data Contributor, and `app_user_group` receives Reader and Storage Blob Data Reader.
...
- When `custom_parameters.storage_account_name` is provided, the module grants storage-account scoped RBAC against the Databricks managed resource group's storage account path.
