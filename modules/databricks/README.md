...
- Optional default storage account RBAC when both `custom_parameters.storage_account_name` and `assign_app_groups_to_managed_storage_account = true` are set.
...
- Managed storage RBAC is explicit opt-in because Azure deny assignments on Databricks managed resource groups can prevent role assignment creation in some environments.
- Pass `inherited_resource_group_tags` when the parent composition already knows resource-group tags; explicit `tags` take precedence.
