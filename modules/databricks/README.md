...
Provisions an Azure Databricks workspace with secure network defaults, optional VNet injection, access connector, customer-managed encryption, private endpoints, diagnostics, and Azure RBAC.

## Features

- Disables public network access by default.
- Supports Premium workspace security, compliance, and encryption features.
- Supports VNet injection with no-public-IP clusters and caller-owned subnets.
- Creates an optional Databricks access connector for Unity Catalog and storage firewall scenarios.
- Supports managed disk, managed services, and root DBFS customer-managed keys.
- Creates optional private endpoints for workspace UI/API and browser authentication.
- Supports Azure Monitor diagnostics and workspace- or storage-scoped role assignments.
- Supports explicit or generated globally unique naming and resource-group tag inheritance.

## Resources Created

The module always creates one `azurerm_databricks_workspace`. It conditionally creates:

- A Databricks access connector and its role assignments.
- A root DBFS customer-managed-key resource.
- Private endpoints and private DNS zone associations.
- Workspace, managed-storage, and access-connector Azure RBAC assignments.
- An Azure Monitor diagnostic setting.
- A random suffix for generated names.

Azure creates the managed resource group, managed storage, and other workspace infrastructure indirectly. Existing virtual networks, subnets, NSGs, private DNS zones, identities, Key Vault keys, monitoring destinations, and Microsoft Entra groups remain caller-owned.

## Prerequisites and Dependencies

- An existing resource group.
- Premium SKU for customer-managed keys, infrastructure encryption, enhanced security monitoring, or compliance security profiles.
- Two correctly sized Databricks VNet-injection subnets and their NSG association IDs when custom networking is enabled.
- An existing private-endpoint subnet and `privatelink.azuredatabricks.net` DNS design for private access.
- Existing Key Vault keys and required key permissions for customer-managed encryption.
- Existing Log Analytics, Storage, or Event Hub destinations for diagnostics.
- Regional service capacity, Databricks account entitlements, and any required Marketplace/provider registration.
...
AzureAD resolves optional group assignments. The Terraform identity needs permission to manage the workspace, managed resource group relationship, private endpoints, diagnostics, and any configured role assignments.

## Basic Usage

See the executable [basic example](examples/basic/), [complete example](examples/complete/), and [access connector example](examples/unity-catalog-access-connector/).

```hcl
module "databricks" {
  source = "../../modules/databricks"

  name                          = "dbw-lakehouse-prod-001"
...
- A private-by-default workspace without VNet injection or private endpoints is not yet a complete connectivity design.
- Premium-only features are rejected when the workspace uses another SKU.
- `create_access_connector` and `access_connector_id` represent mutually exclusive ownership paths.
- Customer-managed-key inputs require coordinated Key Vault permissions and can introduce replacement-sensitive or availability-sensitive changes.
- Managed storage role assignments are explicit opt-in because deny assignments on the Databricks managed resource group can prevent their creation.
- Workspace, private endpoint, enhanced security, and diagnostics features materially affect cost.

## Networking and Private Connectivity

VNet injection uses caller-owned public and private subnets plus their NSG associations. Set `no_public_ip = true` to prevent cluster nodes from receiving public IPs. Routing, NAT, service endpoints, private DNS, NSGs, and required Databricks egress remain outside the module.

Private endpoints are separate from VNet injection. `databricks_ui_api` provides private workspace access. `browser_authentication` has regional and multi-workspace design implications; coordinate it with the platform DNS and authentication architecture.

## Identity and RBAC

The optional access connector can use system-assigned or user-assigned identity and receive role assignments on external storage. The module also supports workspace-scoped assignments and optional group assignments.

Prefer immutable Microsoft Entra object IDs over display names. Databricks workspace-level identities, groups, service principals, Unity Catalog privileges, and cluster policies are data-plane concerns outside this ARM module.

## Naming and Tagging

Set `name` explicitly or generate it from workload, environment, location, and instance inputs with an optional random suffix. Explicit tags override inherited resource-group tags.

## Architecture

See [Architecture](docs/architecture.md) for managed-resource boundaries, VNet injection, private endpoints, identity, encryption, and monitoring relationships.
...
Validate each executable example independently before use.

## Known Limitations

- The module does not manage clusters, pools, notebooks, jobs, workspace users, cluster policies, secret scopes, Unity Catalog metastores, or data-plane grants.
- Workspace provisioning does not prove Databricks account entitlements, network reachability, quota, or Key Vault permissions.
- Managed resource group contents are created by Azure and should not be independently managed.
- Browser-authentication private endpoints require an organization-level design when multiple workspaces share a region.
...
| [azurerm_role_assignment.app_admin_group_storage_account_blob_data_contributor](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/role_assignment) | resource |
| [azurerm_role_assignment.app_admin_group_storage_account_contributor](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/role_assignment) | resource |
...
| [azurerm_role_assignment.app_user_group_storage_account_blob_data_reader](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/role_assignment) | resource |
| [azurerm_role_assignment.app_user_group_storage_account_reader](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/role_assignment) | resource |
...
| <a name="input_assign_app_groups_to_managed_storage_account"></a> [assign\_app\_groups\_to\_managed\_storage\_account](#input\_assign\_app\_groups\_to\_managed\_storage\_account) | Whether app\_admin\_group and app\_user\_group receive RBAC on the custom Databricks managed storage account. This is opt-in because managed resource group deny assignments may prevent role assignment creation. | `bool` | `false` | no |
...
| <a name="output_app_admin_group_storage_account_blob_data_contributor_role_assignment_ids"></a> [app\_admin\_group\_storage\_account\_blob\_data\_contributor\_role\_assignment\_ids](#output\_app\_admin\_group\_storage\_account\_blob\_data\_contributor\_role\_assignment\_ids) | Storage Blob Data Contributor role assignment IDs keyed by principal ID. |
| <a name="output_app_admin_group_storage_account_contributor_role_assignment_ids"></a> [app\_admin\_group\_storage\_account\_contributor\_role\_assignment\_ids](#output\_app\_admin\_group\_storage\_account\_contributor\_role\_assignment\_ids) | Storage account Contributor role assignment IDs keyed by principal ID. |
...
| <a name="output_app_user_group_storage_account_blob_data_reader_role_assignment_ids"></a> [app\_user\_group\_storage\_account\_blob\_data\_reader\_role\_assignment\_ids](#output\_app\_user\_group\_storage\_account\_blob\_data\_reader\_role\_assignment\_ids) | Storage Blob Data Reader role assignment IDs keyed by principal ID. |
| <a name="output_app_user_group_storage_account_reader_role_assignment_ids"></a> [app\_user\_group\_storage\_account\_reader\_role\_assignment\_ids](#output\_app\_user\_group\_storage\_account\_reader\_role\_assignment\_ids) | Storage account Reader role assignment IDs keyed by principal ID. |
