...
- `custom_parameters.storage_account_name`: optional Databricks default storage account name.
- `assign_app_groups_to_managed_storage_account`: explicit opt-in for app-group RBAC on that managed storage account; managed-resource-group deny assignments may block it.
...
- `app_admin_group`, `app_user_group`, `role_assignments`, `access_connector_role_assignments`: RBAC controls. App admins receive workspace Contributor and app users receive workspace Reader.
