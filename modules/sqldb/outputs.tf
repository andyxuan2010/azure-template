...
output "free_limit_configuration" {
  description = "Azure SQL free monthly limit configuration summary."
  value = {
    enabled             = var.use_free_limit
    exhaustion_behavior = var.use_free_limit ? var.free_limit_exhaustion_behavior : null
    update_resource_id  = try(azapi_update_resource.free_limit[0].id, null)
  }
}
