...
- `use_free_limit`, `free_limit_exhaustion_behavior`: Azure SQL free monthly limits for eligible serverless databases.
- With `use_free_limit = true` and `free_limit_exhaustion_behavior = "AutoPause"`, the module lets Azure use the default max size because custom max sizes are rejected by the API.
