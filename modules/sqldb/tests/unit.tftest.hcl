...
run "plan_secure_defaults" {
  ...
  assert {
    condition     = output.tags.Owner == "CCOE" && !contains(keys(output.tags), "Environment") && !contains(keys(output.tags), "ManagedBy") && !contains(keys(output.tags), "module")
    error_message = "SQL DB should preserve caller tags without generating implicit tags."
  }
}
...
run "plan_generated_names_without_random" {
  ...
  assert {
    condition     = output.server_name == "sql-platform-poc-cc-001" && output.database_name == "sqldb-platform-poc-cc-001"
...
run "reject_ambiguous_custom_role_definition" {
  command = plan
...
run "reject_invalid_custom_role_scope" {
  command = plan
