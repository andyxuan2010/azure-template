...
variable "backup_storage_redundancy" {
  description = "Backup storage redundancy for the SQL Database. Valid values are Local, Zone, or Geo."
  type        = string
  default     = "Local"
  validation {
    condition     = contains(["Local", "Zone", "Geo"], var.backup_storage_redundancy)
    error_message = "backup_storage_redundancy must be one of Local, Zone, or Geo."
  }
}
...
variable "admin_username" {
  ...
  validation {
    ...
    error_message = "Admin username must be null, empty, or start with a letter and be 1-128 chars."
  }
  sensitive = true
}
...
variable "admin_password" {
  ...
  validation {
    ...
    error_message = "Admin password must be null, empty, or at least 8 characters long."
  }
  sensitive = true
}
...
variable "admin_credentials_key_vault_id" {
  type        = string
  default     = ""
  description = "Optional Azure Key Vault resource ID containing the SQL admin username and password secrets."
...
variable "ad_admin_login_name" {
  ...
  type        = string
  validation {
    condition     = length(var.ad_admin_login_name) > 0
...
variable "sku_name" {
  description = "SKU for the SQL Database (e.g., Basic, S0, S1, P1, GP_Gen5_2)"
  type        = string
  validation {
    condition     = can(regex("^(Basic|S[0-2]|P[1-6]|GP_Gen[45]|BC_Gen[45]|HS_Gen[45]|DW|DC)\\d*$", var.sku_name))
    error_message = "SKU must be a valid SQL Database SKU (e.g., Basic, S0, S1, P1, GP_Gen5_2)."
  }
}
...
variable "resource_group_name" {
  ...
  type        = string
  validation {
    condition     = trimspace(var.resource_group_name) != "" && can(regex("^[a-zA-Z0-9._()-]{1,90}$", var.resource_group_name))
...
variable "firewall_rules" {
  ...
  validation {
    ...
    error_message = "Each firewall_rules entry must have a non-empty name and IPv4 start_ip_address/end_ip_address values."
  }
}
...
variable "enable_threat_detection" {
  description = "Enable Microsoft Defender for SQL threat detection for the server."
  type        = bool
  default     = false
}
