...
run "plan_generated_name_for_dependent_resources" {
  command = plan
  ...
  assert {
    condition = (
      azurerm_private_endpoint.this[0].name == "pep-${output.name}-sites" &&
      azurerm_monitor_diagnostic_setting.this[0].name == "${output.name}-diagnostic-setting"
    )
    error_message = "Dependent resource names should use the resolved generated Logic App name."
  }
}
