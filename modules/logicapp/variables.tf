...
variable "inherit_resource_group_tags" {
  type        = bool
  description = "Whether to merge tags from the target resource group into Logic App resources."
  default     = true
}
...
variable "name" {
  type        = string
  description = "Optional Logic App Standard name override. Leave empty to generate one from the naming convention."
  default     = ""

  validation {
    condition     = trimspace(var.name) == "" || can(regex("^[a-z0-9-]{2,60}$", trimspace(var.name)))
    error_message = "name must be empty or 2-60 characters and contain only lowercase letters, numbers, or hyphens."
  }
}
...
variable "connection_strings" {
  ...
  validation {
    ...
    error_message = "connection_strings require non-empty names and supported connection string types."
  }
