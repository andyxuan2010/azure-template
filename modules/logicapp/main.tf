...
#
# Module: logicapp
# Description: Deploys Azure Logic App workflow resources for integration and automation scenarios.
#              The module supports workflow creation, managed identity, parameters, access control, diagnostics, role assignments, optional private endpoint composition, and resource group tag inheritance for governance alignment.
...
#
# Change History:
# - 2026-05-01 v1.0.0: Established reusable logicapp module baseline for the CCOE Azure IaC template library.
...
resource "azurerm_private_endpoint" "this" {
  ...
  name                = "pep-${local.logic_app_name}-sites"
  location            = local.location
  resource_group_name = var.resource_group_name
  subnet_id           = local.private_endpoint_subnet_id_resolved

  private_service_connection {
    name                           = "psc-${local.logic_app_name}-sites"
...
resource "azurerm_monitor_diagnostic_setting" "this" {
  ...
  name                       = "${local.logic_app_name}-diagnostic-setting"
