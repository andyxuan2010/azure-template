...
run "plan" {
  ...
  assert {
    condition     = length(data.azuread_application.this) == 0
    error_message = "The app registration lookup should be skipped when Application Proxy is disabled."
  }
}
...
run "plan_application_proxy" {
  ...
  assert {
    condition     = length(data.azuread_application.this) == 1
    error_message = "Application Proxy should perform exactly one app registration lookup."
  }
}
