...
# Description: Deploys Azure Storage Accounts for blob, file, queue, table, and platform storage scenarios.
#              The module supports account kind and replication settings, network controls, containers and shares where configured, private endpoint integration, diagnostics, role assignments, and resource group tag inheritance for governance alignment.
...
#
# Change History:
# - 2026-04-17 v1.0.0: Established reusable storageaccount module baseline for the CCOE Azure IaC template library.
...
resource "azurerm_role_assignment" "app_admin_group_data_plane" {
  for_each = local.app_admin_group_data_plane_role_assignments
