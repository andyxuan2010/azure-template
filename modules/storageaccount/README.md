...
- Grants the current Terraform execution identity `Contributor`, `Storage Blob Data Contributor`, and `Storage File Data SMB Share Contributor` by default. Set `grant_current_terraform_service_principal_storage_roles = false` to opt out.
...
The module also grants the Terraform execution identity `Contributor`, `Storage Blob Data Contributor`, and `Storage File Data SMB Share Contributor` at the storage account scope by default, which helps the same service principal manage storage control-plane and data-plane resources created by Terraform. Disable this behavior with `grant_current_terraform_service_principal_storage_roles = false` when those permissions are managed elsewhere.
...
- `grant_current_terraform_service_principal_storage_roles`: Assign Contributor, Storage Blob Data Contributor, and Storage File Data SMB Share Contributor to the current Terraform execution identity at the storage account scope. `bool` (default: true)
