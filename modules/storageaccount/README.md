...
- The module exposes the full storage account, data-service, RBAC, networking, private endpoint, and diagnostics interface documented in `variables.tf`.
- Nested modules: 0
...
- Requires callers to pass `azurerm.prod` because the alias is part of the module contract; it is used for private DNS zone lookup by name.
- Uses deterministic mocked plan tests for secure defaults and private endpoint/diagnostics composition.
...
Set `location` and, when inheriting tags, `inherited_resource_group_tags` to plan without reading the resource group. This is useful when the resource group is being created in the same root plan.
