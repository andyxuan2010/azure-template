...
- Always pass `azurerm.prod`. When using DNS lookup by name, also set `private_dns_zone_resource_group_name`.
- Prefer direct subnet and private DNS zone IDs when the dependency resources are already available to the root module.
