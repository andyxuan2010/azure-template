...
Default behavior: public network access is disabled, network rules are enabled with a `Deny` default action, and the current Terraform execution identity receives `Contributor`, `Storage Blob Data Contributor`, and `Storage File Data SMB Share Contributor` at the storage account scope. Supply explicit IP rules, subnet IDs, or private endpoints before expecting connectivity.
