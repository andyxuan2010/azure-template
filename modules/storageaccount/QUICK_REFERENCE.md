...
- `app_admin_group`: `list(string)`; receives Contributor plus Blob, File, Queue, and Table data-plane access
...
- `grant_current_terraform_service_principal_storage_roles`: `bool` (default: `true`)
...
- `app_admin_group_data_plane_role_assignment_ids`
...
- `terraform_execution_identity_role_assignment_ids`
...
Default behavior: public network access is disabled, network rules are enabled with a `Deny` default action, every `app_admin_group` principal receives `Contributor`, `Storage Blob Data Owner`, `Storage File Data SMB Share Elevated Contributor`, `Storage Queue Data Contributor`, and `Storage Table Data Contributor`, and the current Terraform execution identity receives the same full management and data-plane role set at the storage account scope. Supply explicit IP rules, subnet IDs, or private endpoints before expecting connectivity.
