variable "name" {
  type        = string
  description = "Optional SQL Managed Instance name override. Leave empty to generate one from the naming convention."
  default     = ""

  validation {
    condition     = trimspace(var.name) == "" || can(regex("^[a-z0-9-]{1,63}$", trimspace(var.name)))
    error_message = "name must be empty or 1-63 characters and contain only lowercase letters, numbers, and hyphens."
  }
}
...
variable "inherit_resource_group_tags" {
  type        = bool
  description = "Whether to merge tags from the target resource group into SQL Managed Instance resources."
  default     = true
}
...
check "sqlmi_input_consistency" {
  ...
  assert {
    condition     = alltrue([for value in concat(var.app_admin_group, var.app_user_group) : trimspace(value) != ""])
    error_message = "app_admin_group and app_user_group cannot contain empty values."
  }
}
