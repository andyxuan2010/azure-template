...
variable "grant_current_caller_secrets_officer" {
  type        = bool
  description = "Whether to grant the current Terraform caller the Key Vault Secrets Officer role."
  default     = true
}
...
check "keyvault_input_consistency" {
  assert {
    ...
    error_message = "When enable_rbac_authorization is false, app_admin_group, app_user_group, and grant_current_caller_secrets_officer must be disabled because this module does not manage Key Vault access policies."
  }
