...
output "current_caller_secrets_officer_role_assignment_id" {
  description = "The backward-compatible Key Vault Secrets Officer role assignment ID granted to the current Terraform caller when full current-caller Key Vault roles are disabled."
  value       = try(azurerm_role_assignment.current_terraform_service_principal["secrets_officer"].id, null)
}

output "current_terraform_service_principal_role_assignment_ids" {
  description = "Map of role assignment IDs created for the current Terraform execution identity."
  value       = { for k, v in azurerm_role_assignment.current_terraform_service_principal : k => v.id }
}
...
output "certificate_contacts_id" {
  description = "The Key Vault certificate contacts resource ID, if contacts are configured."
