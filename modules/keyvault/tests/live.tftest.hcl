...
run "apply" {
  ...
  assert {
    condition     = output.current_caller_secrets_officer_role_assignment_id != null
    error_message = "Key Vault did not grant the current caller Secrets Officer as expected."
  }

  assert {
    condition     = length(output.current_terraform_service_principal_role_assignment_ids) == 2
    error_message = "Key Vault did not grant Contributor and Secrets Officer to the Terraform execution identity as expected."
  }
}
