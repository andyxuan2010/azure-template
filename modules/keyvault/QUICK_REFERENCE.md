...
Default behavior: public network access is disabled, network ACLs are enabled with a `Deny` default action, and the current Terraform execution identity receives `Contributor` and `Key Vault Administrator` at the Key Vault scope. Supply explicit IP rules, subnet IDs, or a private endpoint before expecting connectivity.
