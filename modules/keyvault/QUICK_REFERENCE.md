...
- `grant_current_terraform_service_principal_key_vault_roles`: `bool` (default: `true`)
- `grant_current_caller_secrets_officer`: `bool` (default: `true`)
...
Default behavior: public network access is disabled, network ACLs are enabled with a `Deny` default action, and the current Terraform execution identity receives `Contributor` and `Key Vault Secrets Officer` at the Key Vault scope. Supply explicit IP rules, subnet IDs, or a private endpoint before expecting connectivity.
