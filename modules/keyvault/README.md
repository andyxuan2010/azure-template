...
- Grants the current Terraform execution identity `Contributor` and `Key Vault Secrets Officer` by default. Set `grant_current_terraform_service_principal_key_vault_roles = false` to opt out.
...
By default, the module grants the Terraform execution identity `Contributor` and `Key Vault Secrets Officer` at the Key Vault scope, which helps the same service principal manage the vault and secrets created by Terraform. Disable this behavior with `grant_current_terraform_service_principal_key_vault_roles = false` when those permissions are managed elsewhere.
...
- `grant_current_terraform_service_principal_key_vault_roles`: Assign Contributor and Key Vault Secrets Officer to the current Terraform execution identity at the Key Vault scope. `bool` (default: true)
