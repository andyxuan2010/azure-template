...
- Optionally grants the current Terraform caller the `Key Vault Secrets Officer` role on the vault.
...
The module can also grant the current Terraform caller `Key Vault Secrets Officer` access on the created vault through `grant_current_caller_secrets_officer`.
...
- `grant_current_caller_secrets_officer`: Whether to grant the current caller the `Key Vault Secrets Officer` role. `bool` (default: true)
