...
- Grants the current Terraform execution identity `Contributor` and `Key Vault Administrator` by default for control-plane and data-plane access. Set `grant_current_terraform_service_principal_key_vault_roles = false` to opt out.
...
By default, the module grants the Terraform execution identity `Contributor` and `Key Vault Administrator` at the Key Vault scope, which helps the same service principal manage the vault control plane and Key Vault data plane resources created by Terraform. Disable this behavior with `grant_current_terraform_service_principal_key_vault_roles = false` when those permissions are managed elsewhere.
...
- `grant_current_terraform_service_principal_key_vault_roles`: Assign Contributor and Key Vault Administrator to the current Terraform execution identity at the Key Vault scope. `bool` (default: true)
- `grant_current_caller_secrets_officer`: Backward-compatible flag for granting only `Key Vault Secrets Officer` when `grant_current_terraform_service_principal_key_vault_roles` is false. `bool` (default: true)
