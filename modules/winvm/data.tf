...
data "azurerm_key_vault_secret" "domain-join-password" {
  count        = var.enable_domain_join && var.app_env != "sbx" ? 1 : 0
...
data "azuread_group" "app_admin_group_by_name" {
  for_each = {
    for group in var.app_admin_group : group => group
...
data "azuread_group" "app_user_group_by_name" {
  for_each = {
    for group in var.app_user_group : group => group
    if !can(regex("^[0-9a-fA-F-]{36}$", group))
  }
