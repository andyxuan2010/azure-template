...
6. Optionally remove a matching stale AD computer account and join Active Directory through the
   supplied domain controller after pre-checks. Cleanup runs only while the VM is in a workgroup
   and uses the domain-join credential; the credential must be allowed to delete the computer
   object.
7. If `PrimaryDnsSuffix` is supplied, apply the Windows primary DNS suffix after the domain-join
   step. An empty value leaves the existing suffix unchanged; a changed suffix schedules a reboot
   unless `RebootWhenDone` already requests one.
8. Optional Defender performance mode.
9. Install enabled baseline tools: PowerShell 7, Azure CLI, 7-Zip, standalone BGInfo.
10. Update PATH.
11. Configure OpenSSH and `administrators_authorized_keys`.
12. Configure BGInfo and the background wallpaper.
13. Register `scheduled.ps1` idle task.
14. Stage `validation.ps1`.
15. Clean Azure Arc surfacing.
16. Configure WinRM securely.
17. Add local group memberships.
18. Optionally install/register SHIR.
19. Finalize.
...
password is converted to a `SecureString` before `Add-Computer` is called. Before `Add-Computer`,
the script searches the selected domain controller for a computer object whose `sAMAccountName`
matches the VM name and deletes it when found. This avoids the AD "same name exists" / blocked
account-reuse failure after an earlier domain leave. The cleanup is performed through LDAP/ADSI
and does not require the RSAT Active Directory PowerShell module. The `JsonADDomainExtension`
path does not use this `init2.ps1` cleanup behavior. When `primary_dns_suffix` is non-empty and a
bootstrap execution method is enabled, the module passes it as `PrimaryDnsSuffix` to
`init2.ps1`; the script updates `NV Domain` and `Domain` under
`HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters` after the domain-join step and
schedules a delayed restart when the value changes. An empty input is skipped. If bootstrap is
disabled, the dedicated suffix Run Command applies the same registry settings.
