output "privateips" {
  description = "Private IP addresses for all Windows VMs in index order."
  value       = azurerm_network_interface.this[*].private_ip_address
}

output "network_interface_ids" {
  description = "Network interface IDs for all Windows VMs in index order."
  value       = azurerm_network_interface.this[*].id
}

output "network_interface_ip_configuration_names" {
  description = "Primary NIC IP configuration names for all Windows VMs in index order."
  value       = [for nic in azurerm_network_interface.this : nic.ip_configuration[0].name]
}
...
output "vm_ids" {
  description = "Windows VM resource IDs in index order."
  value       = azurerm_windows_virtual_machine.this[*].id
}

output "principal_ids" {
  description = "System-assigned managed identity principal IDs for the Windows VMs."
  value       = azurerm_windows_virtual_machine.this[*].identity[0].principal_id
}

output "merged_tags" {
  description = "Final merged tags applied to Windows VM resources."
  value       = local.tags
}
