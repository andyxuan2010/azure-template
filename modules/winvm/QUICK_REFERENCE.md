...
- `run_command_replace_trigger`: `string`; optional external trigger value for rerunning Run Command bootstrap when storage-backed scripts change. The module also reruns Run Command when bootstrap arguments, resolved Windows access groups, or bundled bootstrap script content changes.
...
- GUID group values are resolved to Entra group display names before being passed to `init2.ps1` for local group membership. `windows_group_domain_prefix` prefixes bare names only and is normalized to one separator.
