...
function Enter-DefenderPerfMode {
    ...
    if (-not $EnableDefenderPerformanceMode) {
        Write-Log "Defender perf mode disabled by parameter; skipping."
        return
    }
...
function Exit-DefenderPerfMode {
    ...
    if (-not $EnableDefenderPerformanceMode) { return }
    if (-not (Test-DefenderAvailable)) { return }
    try {
...
if ($EnableSHIR) {
    ...
    try {
        ...
        if (-not $shirInstalled) {
            ...
            #$gatewayKey = az keyvault secret show --vault-name "kv-ccoe-cc-${Env}" --name "shir-iactest-dev-601-shir-key1" --query value -o tsv
...
if ($EnableSHIR) {
    try {
        #$gatewayKey = (Get-AzKeyVaultSecret -VaultName "kv-ccoe-cc-${sub}" -Name "adf-ccoe-shir-default-key").SecretValueText
        $gatewayKey = Get-AzKeyVaultSecret -VaultName "kv-ccoe-cc-${sub}" -Name "adf-ccoe-shir-default-key" -AsPlainText -ErrorAction Stop
        # Get-KVSecretText -Name 'adf-ccoe-shir-default-key'
        if ([string]::IsNullOrWhiteSpace($gatewayKey)) {
            Write-Log "Gateway key from Key Vault is empty" -Level Error
            throw "No valid gateway key found"
        }
        #if (-not $gatewayKey) { throw "SHIR gateway key not found in Key Vault." }

        try {
            $factoryName = ($gatewayKey -split '@') | Select-Object -Index 2
            if ($factoryName) {
                $factory = Get-AzResource -Name $factoryName -ResourceType "Microsoft.DataFactory/factories" -ErrorAction SilentlyContinue
                if ($factory) {
                    $rg   = $factory.ResourceGroupName
                    $irs  = Get-AzDataFactoryV2IntegrationRuntime -ResourceGroupName $rg -DataFactoryName $factoryName | Where-Object { $_.Type -eq "SelfHosted" }
                    foreach ($ir in $irs) {
                        $st = Get-AzDataFactoryV2IntegrationRuntime -ResourceGroupName $rg -DataFactoryName $factoryName -Name $ir.Name -Status
                        $node = $st.Nodes | Where-Object { $_.Name -eq $env:COMPUTERNAME -or $_.NodeName -eq $env:COMPUTERNAME }
                        if ($node) {
                            Write-Log "Removing existing SHIR node $($node.NodeName) in $($ir.Name)"
                            Remove-AzDataFactoryV2IntegrationRuntimeNode -ResourceGroupName $rg -DataFactoryName $factoryName -IntegrationRuntimeName $ir.Name -NodeName $env:COMPUTERNAME -Force
                            break
                        }
                    }
                }
            }
        } catch { Write-Log -Message "SHIR pre-cleanup error (non-fatal): $_" -Level WARN }

        $psScriptPath = 'C:\Program Files\Microsoft Integration Runtime\5.0\PowerShellScript\RegisterIntegrationRuntime.ps1'

        if (Test-Path $psScriptPath) {
            try {
                Write-Log "Using official registration script: $psScriptPath"
                Write-Log "The key is: $gatewayKey and the node name is: $($env:COMPUTERNAME)"
                & $psScriptPath -gatewayKey $gatewayKey -NodeName $env:COMPUTERNAME
                Write-Log "SHIR registration completed successfully using official script"
            } catch {
                Write-Log "Error during SHIR registration: $($_.Exception.Message)" -Level Error
                throw
            }
        } else {
            Write-Log "Official registration script not found at: $psScriptPath" -Level Error
            throw "Integration Runtime 5.0 not properly installed"
        }
    }
    catch { Write-Log -Message "SHIR register error: $_" -Level ERROR }
}
