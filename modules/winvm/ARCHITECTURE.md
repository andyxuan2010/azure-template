...
Public networking adds an inbound TCP/3389 rule from `*`. In normal private operations, prefer private access paths such as Bastion, VPN, or ExpressRoute instead of enabling public networking.
...
Domain join credentials use the same direct-input-first pattern with `domain_join_user`, `domain_join_password`, `domain_join_username_secret_name`, and `domain_join_password_secret_name`.
...
Domain join uses `JsonADDomainExtension`, not `init2.ps1`.
...
The module exposes `enable_diagnostics` and `log_analytics_workspace_id`, but the diagnostic setting resource is intentionally omitted because AzureRM provider schema differences vary. Add a provider-compatible `azurerm_monitor_diagnostic_setting` outside this module when diagnostics are required.
