locals {
  ...
  admin_username_effective = trimspace(var.azure-user) != "" ? trimspace(var.azure-user) : data.azurerm_key_vault_secret.azure-user[0].value
  admin_password_effective = trimspace(var.azure-password) != "" ? trimspace(var.azure-password) : data.azurerm_key_vault_secret.azure-password[0].value
  instance_suffixes = {
...
resource "azurerm_virtual_machine_extension" "domain_join_ext" {
  ...
  protected_settings = jsonencode({
    Password = data.azurerm_key_vault_secret.domain-join-password[0].value
  })
...
resource "azurerm_windows_virtual_machine" "this" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = "azure-user must be provided for winvm."
    }

    precondition {
      condition     = local.admin_password_effective != ""
      error_message = "azure-password must be provided for winvm."
    }
  }
}
...
resource "azurerm_virtual_machine_extension" "CustomScriptInit" {
  ...
  # "commandToExecute": "powershell -ExecutionPolicy Bypass -File init.ps1 -Env ${var.app_env} -RebootWhenDone -AppRemoteGroup ${local.app_remote_group_list} -AppAdminGroup ${local.app_admin_group_list} -StorageAccount ${data.azurerm_storage_account.iac.name}"
  # "managedIdentity" : { "clientId": "${azurerm_windows_virtual_machine.this[count.index].identity.0.principal_id}" }

  # "commandToExecute": "powershell -ExecutionPolicy Bypass -File c:\\users\\azureadmin\\downloads\\script.ps1 -Env ${var.app_env} -RebootWhenDone -AppRemoteGroup ${local.app_remote_group_list} -AppAdminGroup ${local.app_admin_group_list} -StorageAccount ${data.azurerm_storage_account.iac.name}",
  ...
  # work example
  # "commandToExecute": "powershell -ExecutionPolicy Bypass -Command \"Set-ExecutionPolicy RemoteSigned -Force; Copy-Item 'c:\\AzureData\\CustomData.bin' 'c:\\AzureData\\script.ps1' -Force; & 'c:\\AzureData\\script.ps1' -Env ${var.app_env} -RebootWhenDone -AppRemoteGroup ${local.app_remote_group_list} -AppAdminGroup ${local.app_admin_group_list} -StorageAccount ${data.azurerm_storage_account.iac.name} \""

  settings = <<SETTINGS
  {
    "commandToExecute": "powershell -ExecutionPolicy Bypass -Command \"Set-ExecutionPolicy RemoteSigned -Force; Copy-Item 'c:\\AzureData\\CustomData.bin' 'c:\\AzureData\\script.ps1' -Force; & 'c:\\AzureData\\script.ps1' ${local.enable_shir_flag} -Env ${var.app_env} -RebootWhenDone -StorageAccount ${data.azurerm_storage_account.iac.name} \""
  }
  ...
  #tags = var.rg_tags
  tags = merge(data.azurerm_resource_group.app.tags, var.tags)
  depends_on = [
    azurerm_virtual_machine_extension.NetworkWatcherAgentWindows,
    azurerm_virtual_machine_extension.aad_login,
...
resource "azurerm_role_assignment" "vm2st" {
  ...
  depends_on           = [data.azurerm_storage_account.iac]
}
...
resource "azurerm_role_assignment" "vm2kv" {
  ...
  principal_id         = azurerm_windows_virtual_machine.this[count.index].identity.0.principal_id
  depends_on           = [data.azurerm_key_vault.iac]
}
