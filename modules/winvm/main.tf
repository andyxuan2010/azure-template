...
#
# Module: winvm
# Description: Deploys one or more Windows virtual machines for application or platform workloads on Azure.
#              The module supports generated or explicit VM names, NICs, managed disks, availability sets or zones, domain join, initialization scripts, managed identity and access assignments, and resource group tag inheritance for governance alignment.
...
#
# Change History:
# - 2026-04-06 v1.0.0: Established reusable winvm module baseline for the CCOE Azure IaC template library.
...
locals {
  ...
  location_code          = lookup(local.location_code_map, lower(trimspace(var.location)), lower(substr(join("", regexall("[a-z0-9]", replace(var.location, " ", ""))), 0, 3)))
  workload_code          = substr(lower(join("", regexall("[a-z0-9]", trimspace(var.workload)))), 0, 4)
  environment_code       = lookup(local.env_code_map, var.app_env, substr(var.app_env, 0, 1))
  generated_vm_name_base = substr("w${local.workload_code}${local.location_code}${local.environment_code}", 0, 12)
  vm_name_base           = trimspace(var.name) != "" ? trimspace(var.name) : (trimspace(var.app_vm) != "" ? trimspace(var.app_vm) : local.generated_vm_name_base)
  instance_suffixes = {
...
resource "azurerm_network_interface" "this" {
  count               = var.app_vm_number
  name                = "nic-${local.vm_name_base}${local.instance_suffixes[count.index]}"
  location            = var.location
  resource_group_name = var.app_rg

  ip_configuration {
    name                          = "nicconfig-${local.vm_name_base}${local.instance_suffixes[count.index]}"
...
resource "azurerm_virtual_machine_run_command" "primary_dns_suffix" {
  count              = local.primary_dns_suffix_effective != "" && (var.enable_domain_join || local.init2_domain_join_active) && !var.enable_custom_script_extension && !var.enable_virtual_machine_run_command && var.app_env != "sbx" ? var.app_vm_number : 0
  name               = "ConfigurePrimaryDnsSuffix"
  location           = var.location
  virtual_machine_id = azurerm_windows_virtual_machine.this[count.index].id

  source {
    script = local.primary_dns_suffix_script
  }

  tags = local.tags
  depends_on = [
    azurerm_virtual_machine_extension.domain_join_ext,
    azurerm_virtual_machine_extension.CustomScriptInit,
    azurerm_virtual_machine_run_command.Init,
  ]
}
...
resource "azurerm_windows_virtual_machine" "this" {
  count               = var.app_vm_number
  name                = "${local.vm_name_base}${local.instance_suffixes[count.index]}"
  computer_name       = "${local.vm_name_base}${local.instance_suffixes[count.index]}"
  resource_group_name = var.app_rg
  location            = var.location
  ...
  lifecycle {
    ...
    precondition {
      condition = !local.init2_domain_join_active || alltrue([
        trimspace(var.init2_domain_name) != "",
        trimspace(var.init2_domain_controller) != "",
        trimspace(var.init2_domain_join_vault_name) != "",
        trimspace(var.init2_domain_join_password_secret_name) != "",
      ])
      error_message = "When init2_enable_domain_join is true and enable_domain_join is false, init2_domain_name, init2_domain_controller, init2_domain_join_vault_name, and init2_domain_join_password_secret_name must all be non-empty."
    }
  }
}
...
resource "azurerm_managed_disk" "this" {
  ...
  name                  = "disk2-${local.vm_name_base}${local.instance_suffixes[count.index]}"
  location              = var.location
  resource_group_name   = var.app_rg
  zone                  = local.vm_zones[count.index]
  storage_account_type  = "Standard_LRS"
...
locals {
  ...
  bootstrap_init_arguments = compact([
    ...
    local.init2_domain_join_active ? "-EnableDomainJoin" : "",
    local.init2_domain_join_active ? "-DomainName '${replace(var.init2_domain_name, "'", "''")}'" : "",
    local.init2_domain_join_active ? "-DomainController '${replace(var.init2_domain_controller, "'", "''")}'" : "",
    local.init2_domain_join_active ? "-DomainJoinVaultName '${replace(var.init2_domain_join_vault_name, "'", "''")}'" : "",
    local.init2_domain_join_active ? "-DomainJoinPasswordSecretName '${replace(var.init2_domain_join_password_secret_name, "'", "''")}'" : "",
    local.primary_dns_suffix_effective != "" ? "-PrimaryDnsSuffix '${replace(local.primary_dns_suffix_effective, "'", "''")}'" : "",
  ...
  # Keep Run Command synchronous until completion is reported. Passing
  # -RebootWhenDone can interrupt Azure's completion polling and cause Terraform
  # to time out even after the init script has completed. init2.ps1 still
  # schedules a delayed reboot when this switch is omitted.
  run_command_init_arguments = concat(
    var.enable_shir ? ["-EnableSHIR"] : [],
    var.enable_defender_performance_mode ? ["-EnableDefenderPerformanceMode"] : [],
    local.init2_domain_join_active ? [
      "-EnableDomainJoin",
      "-DomainName", var.init2_domain_name,
      "-DomainController", var.init2_domain_controller,
      "-DomainJoinVaultName", var.init2_domain_join_vault_name,
      "-DomainJoinPasswordSecretName", var.init2_domain_join_password_secret_name,
    ] : [],
    local.primary_dns_suffix_effective != "" ? ["-PrimaryDnsSuffix", local.primary_dns_suffix_effective] : [],
    [
      "-Env", var.app_env,
...
resource "azurerm_virtual_machine_extension" "CustomScriptInit" {
  ...
  depends_on = [
    ...
    azurerm_role_assignment.vm2kvsecrets
  ]
...
resource "terraform_data" "run_command_replace_trigger" {
  input = sha256(jsonencode({
    external_trigger          = var.run_command_replace_trigger
    app_admin_group_csv       = local.app_admin_group_csv
    app_user_group_csv        = local.app_user_group_csv
    run_command_arguments     = local.run_command_init_arguments
    run_command_script_sha256 = sha256(local.run_command_init_script)
  }))
}
...
resource "azurerm_virtual_machine_run_command" "Init" {
  ...
  source {
    script = sensitive(local.run_command_init_script)
  }
...
resource "azurerm_public_ip" "this" {
  ...
  name                = "pip-${local.vm_name_base}${local.instance_suffixes[count.index]}"
  location            = var.location
  resource_group_name = var.app_rg
...
resource "azurerm_network_security_group" "this" {
  count               = var.public_network_enabled ? var.app_vm_number : 0
  name                = "nsg-${local.vm_name_base}${local.instance_suffixes[count.index]}"
  location            = var.location
  resource_group_name = var.app_rg
