...
resource "azurerm_virtual_machine_extension" "CustomScriptInit" {
  ...
  settings = <<SETTINGS
  {
    "commandToExecute": "powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command \"Copy-Item 'C:\\AzureData\\CustomData.bin' 'C:\\AzureData\\script.ps1' -Force; & 'C:\\AzureData\\script.ps1' ${local.enable_shir_flag} ${local.enable_defender_performance_mode_flag} -Env '${var.app_env}' -RebootWhenDone -AppRemoteGroup '${local.app_user_group_csv}' -AppAdminGroup '${local.app_admin_group_csv}' -StorageAccount '${local.iac_storage_account_name}'\""
  }
