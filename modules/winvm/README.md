...
Provision Azure Windows virtual machines with managed identity, RBAC, optional domain join, optional AAD login, bootstrap execution, shared storage/script access, local Windows group membership, optional data disks, optional public networking, optional SHIR bootstrap, and optional diagnostics flags.
...
Consumers that store additional VM-specific localization scripts outside this module can pass a content hash to `run_command_replace_trigger`. When that value changes, Terraform replaces the Run Command resource and reruns the bootstrap without replacing the VM.
...
| `run_command_replace_trigger` | string | `""` | Optional external trigger value that forces the Run Command bootstrap resource to be replaced when changed. |
...
| `public_network_enabled` | bool | `false` | Create public IP, NSG, and RDP rule. |
...
| `enable_diagnostics` | bool | `false` | Diagnostics flag. Diagnostic setting resource is intentionally omitted. |
...
| `public_ip` | Public IP address when `public_network_enabled = true`, otherwise `No Public IP Assigned`. |
...
| `diagnostics_enabled` | Whether diagnostics flag was enabled. |
