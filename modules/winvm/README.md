...
| [azurerm_virtual_machine_run_command.primary_dns_suffix](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/virtual_machine_run_command) | resource |
...
| <a name="input_init2_domain_controller"></a> [init2\_domain\_controller](#input\_init2\_domain\_controller) | Fully qualified domain controller name or address passed to init2.ps1 for the optional guest domain join. Required when init2\_enable\_domain\_join is true and enable\_domain\_join is false. | `string` | `""` | no |
| <a name="input_init2_domain_join_password_secret_name"></a> [init2\_domain\_join\_password\_secret\_name](#input\_init2\_domain\_join\_password\_secret\_name) | Key Vault secret name from which init2.ps1 reads the domain join password. Required and validated when init2\_enable\_domain\_join is true and enable\_domain\_join is false. | `string` | `"domain-join-password"` | no |
| <a name="input_init2_domain_join_vault_name"></a> [init2\_domain\_join\_vault\_name](#input\_init2\_domain\_join\_vault\_name) | Key Vault name from which init2.ps1 reads the domain join credentials. Required and validated when init2\_enable\_domain\_join is true and enable\_domain\_join is false. | `string` | `"kv-ccoe-cc-nonprod"` | no |
| <a name="input_init2_domain_name"></a> [init2\_domain\_name](#input\_init2\_domain\_name) | Active Directory domain name passed to init2.ps1 for the optional guest domain join. Required when init2\_enable\_domain\_join is true and enable\_domain\_join is false. | `string` | `""` | no |
| <a name="input_init2_enable_domain_join"></a> [init2\_enable\_domain\_join](#input\_init2\_enable\_domain\_join) | Whether init2.ps1 may perform the optional Active Directory domain join when JsonADDomainExtension is disabled. Ignored when enable\_domain\_join is true. | `bool` | `false` | no |
...
| <a name="input_primary_dns_suffix"></a> [primary\_dns\_suffix](#input\_primary\_dns\_suffix) | Optional Windows primary DNS suffix override applied after Active Directory domain join. Bootstrap passes it to init2.ps1; when bootstrap is disabled, a dedicated Run Command applies it. Leave empty to retain the domain-join default. A reboot is scheduled when the suffix changes. | `string` | `""` | no |
