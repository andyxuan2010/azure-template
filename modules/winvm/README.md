...
- `windows_group_domain_prefix`: Optional Windows domain or NetBIOS prefix applied to bare local group membership names, for example `BBD` produces `BBD\GroupName`. Already-qualified names and SIDs are not changed. `string` (default: `""`)
...
- Object IDs are preferred for Azure RBAC. When an `app_admin_group` or `app_user_group` value is an Entra group object ID, Terraform resolves it to the group display name before passing it to `init2.ps1`. If `windows_group_domain_prefix` is set, Terraform prefixes bare names before passing them to the script. Already-qualified names like `DOMAIN\GroupName` and SIDs are left unchanged.
- The standalone validator is `scripts/validation.ps1`. When the file exists in the configured storage account `scripts` container, `init2.ps1` downloads it to `$env:ProgramData\Bootstrap\validation.ps1` on the VM for future use. Use `-AppAdminGroup` and `-AppUserGroup` to validate the local group outcomes on a provisioned VM.
