...
Lifecycle note: the Run Command resource source includes the current base64 content of `init2.ps1`. Script changes affect the Run Command resource and can be applied without rebuilding the VM. Consumers can also set `run_command_replace_trigger` to a hash of storage-backed localization or VM-specific scripts; when the trigger value changes, Terraform replaces `azurerm_virtual_machine_run_command.Init` and reruns bootstrap. This is one reason Run Command is preferred over Custom Script Extension for this module.
