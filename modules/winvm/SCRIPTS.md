...
| `bbd.png` | BBD wallpaper. Also expected in the storage `scripts` container. |
...
`domain-join.ps1`, `init.ps1`, `script.ps1`, and `install-powershell.ps1` are not part of the normal active sequence. `domain-join.ps1` is a manual repair helper. `init.ps1` and `script.ps1` are retained older/full bootstrap references and include historical GUI package install logic that is not used by the active `init2.ps1` and `scheduled.ps1` path.
...
| Script update behavior | VM `custom_data` drift is ignored to avoid VM rebuilds, so existing VMs may not rerun when `init2.ps1` changes | Run Command source changes when `init2.ps1` changes and can update/rerun independently of VM replacement |
...
Lifecycle note: the Run Command resource source includes the current base64 content of `init2.ps1`. Script changes affect the Run Command resource and can be applied without rebuilding the VM. This is one reason Run Command is preferred over Custom Script Extension for this module.
...
6. Optional Defender performance mode.
7. Install enabled baseline tools: PowerShell 7, Azure CLI, 7-Zip, standalone BGInfo.
8. Update PATH.
9. Configure OpenSSH and `administrators_authorized_keys`.
10. Configure BGInfo and BBD wallpaper.
11. Register `scheduled.ps1` idle task.
12. Stage `validation.ps1`.
13. Clean Azure Arc surfacing.
14. Configure WinRM securely.
15. Add local group memberships.
16. Optionally install/register SHIR.
17. Finalize.
...
| BBD wallpaper | `C:\Windows\Web\Wallpaper\BBD\bbd.png` |
...
The bootstrap downloads `bbd.png`, sets it as the base wallpaper, and runs BGInfo. BGInfo then generates `C:\Utils\BGInfo.bmp` and sets it as the final wallpaper with system information overlaid.
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File C:\ProgramData\Bootstrap\Run-BGInfo.ps1 `
  -WallpaperScript C:\ProgramData\Bootstrap\Set-BBDWallpaper.ps1 `
  -WallpaperPath C:\Windows\Web\Wallpaper\BBD\bbd.png `
...
Normal Terraform provisioning should use `JsonADDomainExtension` rather than this helper.
