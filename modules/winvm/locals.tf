locals {
  ...
  # Keep for_each keys derived only from module inputs. The corresponding
  # principal IDs may be unknown until the AzureAD data sources are resolved,
  # but Terraform can still plan them when they are map values.
  vm_login_admin_group_keys = distinct(concat(
    local.app_admin_group_values,
    local.vm_admin_group_value == "" ? [] : ["vm_admin_group"]
  ))
  vm_login_user_group_keys = distinct(concat(
    local.app_user_group_values,
    local.vm_remote_group_value == "" ? [] : ["vm_remote_group"]
  ))
  vm_login_admin_role_assignments = {
    for pair in setproduct(range(var.app_vm_number), local.vm_login_admin_group_keys) :
    "${pair[0]}|${pair[1]}" => {
      vm_index     = pair[0]
      principal_id = local.vm_login_admin_group_object_ids[pair[1]]
    }
  }
  vm_login_user_role_assignments = {
    for pair in setproduct(range(var.app_vm_number), local.vm_login_user_group_keys) :
    "${pair[0]}|${pair[1]}" => {
      vm_index     = pair[0]
      principal_id = local.vm_login_user_group_object_ids[pair[1]]
    }
  }
  vm_resource_admin_role_assignments = {
    for pair in setproduct(range(var.app_vm_number), distinct(local.app_admin_group_values)) :
    "${pair[0]}|${pair[1]}" => {
      vm_index     = pair[0]
      principal_id = local.app_admin_group_object_ids[pair[1]]
    }
  }
  vm_resource_user_role_assignments = {
    for pair in setproduct(range(var.app_vm_number), distinct(local.app_user_group_values)) :
    "${pair[0]}|${pair[1]}" => {
      vm_index     = pair[0]
      principal_id = local.app_user_group_object_ids[pair[1]]
    }
  }
