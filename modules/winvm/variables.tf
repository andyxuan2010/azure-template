...
check "winvm_input_consistency" {
  assert {
    condition     = trimspace(var.primary_dns_suffix) == "" || (var.enable_domain_join && var.app_env != "sbx")
    error_message = "primary_dns_suffix can only be set when enable_domain_join is true and app_env is not sbx."
  }
