variable "location" {
  ...
  description = "Azure region where the Windows VM resources are created."
}

variable "app_env" {
  type        = string
  default     = "dev"
  description = "Deployment environment used for VM instance suffixes and conditional behavior."
  validation {
...
variable "AADLoginForWindows" {
  description = "Whether to install the Microsoft Entra login extension for Windows. Mutually exclusive with Active Directory domain join."
  default     = true
  type        = bool
}
...
variable "disksize" {
  description = "Size in GiB of one optional managed data disk per VM. Set to 0 to disable data disks."
  type        = number
  default     = 0
}

variable "app_vm_number" {
  description = "Number of Windows VMs and NICs to create."
  type        = number
  default     = 1
...
variable "availability_set_id" {
  ...
  default     = null
  nullable    = true
  description = "Optional Availability Set ID for VM placement. Do not combine with availability zone spread."
...
variable "app_vm_size" {
  description = "Azure VM size used for every Windows VM instance."
  type        = string
...
variable "iac_rg" {
  description = "Existing shared IaC resource group containing the bootstrap Key Vault and Storage account."
  type        = string
...
variable "app_rg" {
  description = "Existing application resource group where Windows VM resources are created."
  type        = string
...
variable "app_snet" {
  description = "Existing subnet name used by the Windows VM network interfaces."
  type        = string
...
variable "app_vnet_rg" {
  description = "Existing resource group containing the application VNet."
  type        = string
...
variable "app_vnet" {
  description = "Existing application VNet name."
  type        = string
...
variable "domain" {
  description = "Active Directory domain name used by JsonADDomainExtension when domain join is enabled."
  type        = string
  default     = "2join.us"
}

variable "init2_enable_domain_join" {
  description = "Whether init2.ps1 may perform the optional Active Directory domain join when JsonADDomainExtension is disabled. Ignored when enable_domain_join is true."
  type        = bool
  default     = false
}

variable "init2_domain_name" {
  description = "Active Directory domain name passed to init2.ps1 for the optional guest domain join. Required when init2_enable_domain_join is true and enable_domain_join is false."
  type        = string
  default     = ""
}

variable "init2_domain_controller" {
  description = "Fully qualified domain controller name or address passed to init2.ps1 for the optional guest domain join. Required when init2_enable_domain_join is true and enable_domain_join is false."
  type        = string
  default     = ""
}

variable "init2_domain_join_vault_name" {
  description = "Key Vault name from which init2.ps1 reads the domain join credentials. Required and validated when init2_enable_domain_join is true and enable_domain_join is false."
  type        = string
  default     = "kv-ccoe-cc-nonprod"
}

variable "init2_domain_join_password_secret_name" {
  description = "Key Vault secret name from which init2.ps1 reads the domain join password. Required and validated when init2_enable_domain_join is true and enable_domain_join is false."
  type        = string
  default     = "domain-join-password"
}

variable "primary_dns_suffix" {
  description = "Optional Windows primary DNS suffix override applied after Active Directory domain join. Bootstrap passes it to init2.ps1; when bootstrap is disabled, a dedicated Run Command applies it. Leave empty to retain the domain-join default. A reboot is scheduled when the suffix changes."
  type        = string
  default     = ""

  validation {
    condition = trimspace(var.primary_dns_suffix) == "" || (
      length(trimspace(var.primary_dns_suffix)) <= 253 &&
      can(regex("^(?i:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)(?:\\.(?i:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?))+$", trimspace(var.primary_dns_suffix)))
    )
    error_message = "primary_dns_suffix must be empty or a valid fully qualified DNS suffix such as ca.aero.bombardier.net."
  }
}
...
variable "vm_remote_group" {
  description = "Optional Microsoft Entra group display name or object ID granted Virtual Machine User Login on each VM."
  type        = string
  default     = null
  nullable    = true
...
variable "vm_admin_group" {
  description = "Optional Microsoft Entra group display name or object ID granted Virtual Machine Administrator Login on each VM."
  type        = string
  default     = null
  nullable    = true
...
variable "public_network_enabled" {
  description = "Whether to create one public IP and source-restricted RDP NSG per VM."
  type        = bool
  default     = false
}
...
variable "tags" {
  description = "Tags merged over inherited application resource-group tags and applied to Windows VM resources."
  type        = map(string)
  default     = {}
...
variable "enable_diagnostics" {
  description = "Whether to create one diagnostic setting per VM targeting Log Analytics."
  type        = bool
  default     = false
}

variable "log_analytics_workspace_id" {
  description = "Log Analytics workspace resource ID required when diagnostics are enabled."
  type        = string
  default     = ""
  validation {
...
check "winvm_input_consistency" {
  assert {
    condition     = trimspace(var.primary_dns_suffix) == "" || ((var.enable_domain_join || var.init2_enable_domain_join) && var.app_env != "sbx")
    error_message = "primary_dns_suffix can only be set when JsonADDomainExtension or the init2 domain join is enabled and app_env is not sbx."
  }
