...
variable "app_vm" {
  ...
  validation {
    condition     = can(regex("^[a-zA-Z0-9-]{1,12}$", trimspace(var.app_vm)))
    error_message = "app_vm must be 1-12 characters using letters, numbers, or hyphens so the module can append a 3-digit instance suffix and keep Windows computer_name within 15 characters."
  }
}
# variable "app_env" {
#   type = string
...
variable "windows_group_domain_prefix" {
  ...
  default     = ""
  nullable    = false
  description = "Optional Windows domain or NetBIOS prefix applied to bare app_admin_group and app_user_group names before local group membership, for example BBD produces BBD\\GroupName. Already-qualified names and SIDs are not changed."
}
