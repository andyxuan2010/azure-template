...
- `tests/live.tftest.hcl` uses mock Azure providers and validates named resources, deterministic generated names, caller-controlled tags, secure defaults, identity, RBAC, diagnostics, private endpoint naming, shared private link resources, high-density scale settings, and firewall validation.
...
Bare IPv4 addresses are normalized to `/32`. Do not set `allowed_ips` while public network access is disabled.
