...
Provision an Azure AI Search service with secure defaults, standardized naming and tags, managed identity, RBAC, private endpoints, shared private link resources, diagnostics, and scale controls.

## Features

- Secure baseline with public network access and local API-key authentication disabled by default.
...
- Built-in Contributor and Reader assignments for Entra groups, plus generic Search service scope RBAC assignments.
- Optional private endpoint with private DNS zone IDs or DNS zone name lookup.
- Shared private link resources for private outbound access to dependency services such as Storage, Key Vault, SQL, Cosmos DB, or OpenAI.
- Diagnostics to Log Analytics, Storage Account archive, and Event Hub.
- Search scale and capability options including replicas, partitions, hosting mode, semantic ranker SKU, firewall IP rules, and trusted Azure services bypass.
...
## Search With Private Dependencies
...
- The `free` SKU does not support private endpoints, IP firewall rules, or `semantic_search_sku`.
- `hosting_mode = "highDensity"` requires `sku = "standard3"` and supports at most 3 partitions.
- `allowed_ips` only applies while `public_network_access_enabled = true`; with public access disabled, private endpoints are the intended inbound path.
- Bare IPv4 firewall entries are normalized to `/32`; Azure AI Search firewall rules do not accept IPv6.
- `customer_managed_key_enforcement_enabled` enforces compliance for non-customer resources, but Azure AI Search CMK key association is not fully represented by this Terraform resource.
- Prefer object IDs over display names in `app_admin_group` and `app_user_group` when names are not unique.
