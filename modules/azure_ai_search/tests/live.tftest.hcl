...
run "plan_named_search_service" {
  ...
  assert {
    ...
    error_message = "Azure AI Search should preserve caller tags without adding module-generated tags."
  }
}

run "plan_public_access_with_bare_ipv4_rule" {
  command = plan
  ...
  assert {
    ...
    error_message = "Public access with a valid bare IPv4 rule should plan successfully."
  }
}

run "reject_ip_rules_when_public_access_disabled" {
  command = plan
...
run "plan_generated_name_without_random" {
  ...
  assert {
    condition     = output.name == "srch-shared-poc-cc-001"
