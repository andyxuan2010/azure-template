# Mock-provider unit plan tests. These runs do not authenticate to Azure or apply resources.
...
run "plan_private_secure_defaults" {
  ...
  assert {
    ...
    error_message = "AKS should preserve caller tags without adding module-generated tags."
  }
}

run "plan_single_character_dns_prefix" {
  command = plan

  variables {
    resource_group_name = "rg-ccoe-iac-cc-dev"
    location            = "canadacentral"
    name                = "aks-iactest-dns-001"
    dns_prefix          = "a"
    app_admin_group     = []
    app_user_group      = []
  }
  ...
  assert {
    condition     = output.name == "aks-iactest-dns-001"
    error_message = "A valid single-character DNS prefix should be accepted."
  }
}
...
run "reject_conflicting_autoscaling_aliases" {
  command = plan
