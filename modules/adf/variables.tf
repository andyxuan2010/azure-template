variable "name" {
  type        = string
  description = "Application or workload token used in the generated Data Factory name."

  validation {
    condition     = can(regex("^[a-zA-Z0-9-]{1,40}$", trimspace(var.name)))
    error_message = "name must be 1-40 alphanumeric characters or hyphens."
  }
}
