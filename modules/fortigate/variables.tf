variable "disable_password_authentication" {
  description = "Override Azure VM password authentication. Null retains legacy behavior (disabled when an SSH key is present). False requires an effective admin password."
  type        = bool
  default     = null
}

variable "system_assigned_identity_enabled" {
  description = "Whether to enable a separate system-assigned managed identity on each FortiGate VM."
  type        = bool
  default     = false
}
...
variable "interface_order" {
  description = "Explicit NIC name order including disabled interfaces (filtered by architecture). Empty preserves legacy primary-first lexical order. Verify Azure and FortiOS mapping before reordering."
  type        = list(string)
  default     = []

  validation {
    condition     = length(var.interface_order) == length(distinct(var.interface_order))
    error_message = "interface_order must not contain duplicate names."
  }
}

variable "interfaces" {
  description = "FortiGate NIC definitions keyed by name. Use interface_order to control attachment order."
  type = map(object({
    role                           = string
