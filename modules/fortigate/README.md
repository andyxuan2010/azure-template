...
- Existing subnet IDs, module-created subnets in a shared VNet, or a
  module-created dedicated VNet and subnets.
...
## Shared Or Dedicated VNet

The module can create the required FortiGate subnets in either a shared
existing VNet or a new dedicated VNet. In both modes, each interface supplies
`subnet_name` and `address_prefixes`.

For a shared VNet, set `create_virtual_network = false`, enable subnet
creation, and identify the existing VNet:
...
For a dedicated VNet, set `create_virtual_network = true` and provide its
address space. The module creates the VNet before creating the same interface
subnets:
...
module "fortigate" {
  ...
  create_subnets                = true
  virtual_network_name          = "vnet-fortigate-prod"
