...
`disable_password_authentication = false` allows Azure VM password authentication
while retaining a configured SSH key. It requires an effective admin password
from an input or Key Vault fallback. The default `null` retains legacy automatic
behavior for other callers. Verify FortiOS `admin-ssh-password` separately; an
Azure provisioning setting is not a live FortiOS authentication-state check.
Changing this setting forces VM replacement in the AzureRM provider; review the
plan and arrange a safe maintenance/migration procedure before applying it.

`interface_order` accepts every interface name exactly once; disabled interfaces
are filtered by architecture and the first enabled interface must be primary.
An empty list retains the legacy primary-first lexical ordering. Supply an
explicit list to preserve attachment order independently of map-key sorting.
Before changing an existing VM's order, verify Azure NIC MAC addresses against
FortiOS physical ports on both appliances. Do not infer FortiOS port numbers
from list position alone. The `interface_order` output exposes effective order.

Set `system_assigned_identity_enabled = true` to enable a separate system-assigned
managed identity on every FortiGate VM, including both instances in active-passive
mode. This defaults to `false` and does not grant Azure roles to the identities.
