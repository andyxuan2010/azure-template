...
run "password_and_key_authentication_ha" {
  command = plan
  variables {
    architecture                    = "active-passive"
    availability_zones              = { a = "1", b = "2" }
    disable_password_authentication = false
    admin_password                  = "TestOnly-NotARealCredential-123!"
  }
  assert {
    condition = alltrue([
      for vm in azurerm_linux_virtual_machine.this :
      !vm.disable_password_authentication && length(vm.admin_ssh_key) == 1
    ])
    error_message = "Both HA VMs must retain SSH keys while allowing password authentication."
  }
}

run "reject_password_auth_without_password" {
  command = plan
  variables {
    disable_password_authentication = false
  }
...
run "explicit_nic_order_ha" {
  command = plan
  variables {
    architecture       = "active-passive"
    availability_zones = { a = "1", b = "2" }
    interface_order    = ["external", "internal", "ha"]
  }
  assert {
    condition     = output.interface_order == ["external", "internal", "ha"]
    error_message = "Explicit NIC order must override lexical map order."
  }
}

run "explicit_nic_order_single" {
  command = plan
  variables {
    interface_order = ["external", "ha", "internal"]
  }
  assert {
    condition     = output.interface_order == ["external", "internal"]
    error_message = "Disabled HA interface must be filtered without reordering other NICs."
  }
}

run "reject_incomplete_nic_order" {
  command = plan
  variables {
    interface_order = ["external"]
  }
  expect_failures = [azurerm_linux_virtual_machine.this]
}

run "reject_nonprimary_first" {
  command = plan
  variables {
    interface_order = ["internal", "external", "ha"]
  }
  expect_failures = [azurerm_linux_virtual_machine.this]
}
...
run "reject_http_probe_without_path" {
  command = plan
...
run "single_with_dedicated_virtual_network" {
  command = plan

  variables {
    create_virtual_network        = true
    create_subnets                = true
    virtual_network_name          = "vnet-fortigate-test"
  ...
  assert {
    condition     = output.virtual_network_name == "vnet-fortigate-test"
    error_message = "Dedicated VNet mode should expose the created VNet name."
  }

  assert {
    condition     = length(output.subnet_ids) == 2
    error_message = "Dedicated VNet mode should create both FortiGate subnets."
  }
}
