output "interface_order" {
  description = "Effective Azure NIC attachment order; not a verified FortiOS port mapping."
...
output "managed_identity_principal_ids" {
  description = "System-assigned managed identity principal IDs keyed by VM suffix; null when disabled."
  value       = { for suffix, vm in azurerm_linux_virtual_machine.this : suffix => try(vm.identity[0].principal_id, null) }
}
