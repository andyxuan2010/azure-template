...
#
# Module: fortigate
# Description: Deploys FortiGate network virtual appliances and supporting Azure networking resources for firewall and security edge scenarios.
#              The module supports single or active-passive architectures, NICs, optional VNet and subnet creation, NSGs, public IPs, internal and external load balancers, VM marketplace images, credential handling, operational tags, and resource group tag inheritance for governance alignment.
...
#
# Change History:
# - 2026-06-23 v1.0.0: Established reusable fortigate module baseline for the CCOE Azure IaC template library.
# - 2026-07-08 v1.0.0: Standardized tag inheritance to merge resource group tags before FortiGate operational and resource-specific tags.
...
resource "azurerm_linux_virtual_machine" "this" {
  ...
  disable_password_authentication = coalesce(var.disable_password_authentication, local.admin_ssh_key_enabled)
  ...
  network_interface_ids = [
    for name in local.interface_order :
  ...
  lifecycle {
    ...
    precondition {
      condition     = length(var.interface_order) == 0 || toset(var.interface_order) == toset(keys(var.interfaces))
      error_message = "interface_order must include every interfaces key exactly once, including disabled interfaces."
    }

    precondition {
      condition     = try(local.enabled_interfaces[local.interface_order[0]].primary, false)
      error_message = "The first enabled interface in interface_order must be primary."
    }
    ...
    precondition {
      condition     = coalesce(var.disable_password_authentication, local.admin_ssh_key_enabled) || local.admin_password_enabled
      error_message = "Password authentication requires admin_password or a readable Key Vault password fallback."
    }
...
resource "azurerm_role_assignment" "vm_app_admin_group" {
  for_each = local.vm_admin_role_assignments

  scope                = azurerm_linux_virtual_machine.this[each.value.instance_suffix].id
  role_definition_name = "Contributor"
  principal_id         = each.value.principal_id
  principal_type       = "Group"
}

resource "azurerm_role_assignment" "nic_app_admin_group" {
  for_each = local.nic_admin_role_assignments

  scope                = azurerm_network_interface.this[each.value.nic_key].id
  role_definition_name = "Contributor"
  principal_id         = each.value.principal_id
  principal_type       = "Group"
}

resource "azurerm_role_assignment" "vm_app_user_group" {
