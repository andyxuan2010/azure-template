resource "azurerm_subnet" "this" {
  ...
  lifecycle {
    precondition {
      condition     = trimspace(var.virtual_network_name) != ""
      error_message = "virtual_network_name is required when create_subnets is true."
    }
...
resource "azurerm_network_security_group" "this" {
  ...
  name                = trimspace(var.network_security_group_name) != "" ? trimspace(var.network_security_group_name) : "nsg-${var.name_prefix}"
...
resource "azurerm_network_interface" "this" {
  ...
  name                           = "${var.name_prefix}-${each.value.instance_suffix}-${each.value.interface_name}-nic"
  location                       = var.location
  resource_group_name            = var.resource_group_name
...
resource "azurerm_linux_virtual_machine" "this" {
  ...
  computer_name       = substr(replace("${var.name_prefix}-${each.value}", "-", ""), 0, 64)
  location            = var.location
  resource_group_name = var.resource_group_name
  size                = var.vm_size
  ...
  disable_password_authentication = trimspace(var.admin_ssh_public_key) != ""
  ...
  dynamic "admin_ssh_key" {
    for_each = trimspace(var.admin_ssh_public_key) != "" ? [var.admin_ssh_public_key] : []
  ...
  lifecycle {
    ...
    precondition {
      condition     = trimspace(var.admin_password) != "" || trimspace(var.admin_ssh_public_key) != ""
      error_message = "Set admin_password or admin_ssh_public_key."
    }
...
resource "azurerm_public_ip" "external_lb" {
  ...
  name                = trimspace(var.external_load_balancer.public_ip_name) != "" ? var.external_load_balancer.public_ip_name : "pip-${var.name_prefix}-external-lb"
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "azurerm_lb" "internal" {
  ...
  name                = trimspace(var.internal_load_balancer.name) != "" ? var.internal_load_balancer.name : "lb-${var.name_prefix}-internal"
  location            = var.location
  resource_group_name = var.resource_group_name
  sku                 = "Standard"
  tags                = local.tags
...
resource "azurerm_lb" "external" {
  ...
  name                = trimspace(var.external_load_balancer.name) != "" ? var.external_load_balancer.name : "lb-${var.name_prefix}-external"
  location            = var.location
  resource_group_name = var.resource_group_name
  sku                 = "Standard"
  tags                = local.tags
