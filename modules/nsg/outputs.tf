...
output "subnet_association_ids" {
  ...
  value = merge(
    ...
    { for k, v in azurerm_subnet_network_security_group_association.by_name : k => v.id }
  )
}
...
output "network_interface_association_ids" {
  ...
  value = merge(
    { for k, v in azurerm_network_interface_security_group_association.this : k => v.id },
    { for k, v in azurerm_network_interface_security_group_association.by_name : k => v.id }
  )
}
