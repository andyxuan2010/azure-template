locals {
  ...
  generated_name        = substr("id-${local.workload_code}-${local.location_code}-${var.app_env}-${trimspace(var.instance)}", 0, 128)
  managed_identity_name = trimspace(var.name) != "" ? trimspace(var.name) : local.generated_name
