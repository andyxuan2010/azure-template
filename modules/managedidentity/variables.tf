variable "name" {
  description = "Optional user-assigned managed identity name override. Leave empty to generate one from the naming convention."
  type        = string
  default     = ""

  validation {
    condition     = trimspace(var.name) == "" || can(regex("^[A-Za-z0-9-_]{3,128}$", trimspace(var.name)))
    error_message = "name must be empty or 3-128 characters using letters, numbers, hyphens, or underscores."
  }
}
