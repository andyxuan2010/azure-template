...
When public networking is enabled, `public_ssh_source_address_prefixes` is mandatory. The module never creates an internet-wide SSH rule.
...
## Example 7: Multiple VMs With Static Private IPs
...
## Example 8: Install First, Migrate Later
...
- Credential precedence is `direct module input -> Key Vault secret -> module default`.
- When you omit `admin_username`, `admin_password`, or `admin_ssh_key`, the module can read them from `admin_credentials_key_vault_id` using `admin_username_secret_name`, `admin_password_secret_name`, and `admin_ssh_key_secret_name`.
...
- `private_ip_addresses = []` keeps dynamic private IP allocation. When static private IPs are needed, provide exactly one IPv4 address per VM in `vm_count` order.
