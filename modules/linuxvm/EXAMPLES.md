...
- Pass `azure-user`, `azure-password`, and `azure-ssh-key` directly; the module does not read these values from Key Vault.
- You can replace `app_vnet` plus `app_vnet_rg` with `app_vnet_id` when you already know the full VNet resource ID.
