...
#
# Module: linuxvm
# Description: Deploys one or more Linux virtual machines for application, platform, or infrastructure workloads on Azure.
#              The module supports generated or explicit VM names, NICs, availability zones, managed disks, cloud-init bootstrap, domain join or localization hooks, identity and access assignments, and resource group tag inheritance for governance alignment.
...
#
# Change History:
# - 2026-03-06 v1.0.0: Established reusable linuxvm module baseline for the CCOE Azure IaC template library.
...
resource "azurerm_network_interface" "this" {
  count               = var.vm_count
  name                = "nic-${local.vm_name_base}${format("%03d", local.suffix_base + count.index)}"
  location            = var.location
...
resource "azurerm_storage_blob" "localization_vm_script" {
  ...
  name                 = each.value
  storage_container_id = local.localization_container_id
  type                 = "Block"
...
resource "azurerm_linux_virtual_machine" "this" {
  count                           = var.vm_count
  name                            = "${local.vm_name_base}${format("%03d", local.suffix_base + count.index)}"
  computer_name                   = "${local.vm_name_base}${format("%03d", local.suffix_base + count.index)}"
  resource_group_name             = var.resource_group_name
  location                        = var.location
...
resource "azurerm_managed_disk" "this" {
  ...
  name                  = "${local.vm_name_base}${format("%03d", local.suffix_base + count.index)}-disk2"
  location              = var.location
  resource_group_name   = var.resource_group_name
  zone                  = local.vm_zones[count.index]
  storage_account_type  = "Standard_LRS"
...
resource "azurerm_public_ip" "this" {
  ...
  name                = "pip-${local.vm_name_base}${format("%03d", local.suffix_base + count.index)}"
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "azurerm_network_security_group" "this" {
  count               = var.public_network_enabled == true ? var.vm_count : 0
  name                = "nsg-${local.vm_name_base}${format("%03d", local.suffix_base + count.index)}"
  location            = var.location
  resource_group_name = var.resource_group_name
