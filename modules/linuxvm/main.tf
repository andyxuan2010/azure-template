...
resource "azurerm_linux_virtual_machine" "this" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = "A non-empty Linux VM admin username is required. Provide azure-user directly or configure admin_credentials_key_vault_id plus admin_username_secret_name when disable_password_authentication is false."
    }

    precondition {
      condition     = var.disable_password_authentication || local.admin_password_effective != null
      error_message = "A non-empty Linux VM admin password is required when disable_password_authentication is false. Provide azure-password directly or configure admin_credentials_key_vault_id plus admin_password_secret_name."
    }

    precondition {
      condition     = local.admin_ssh_key_effective != null
      error_message = "A non-empty Linux VM admin SSH public key is required. Provide azure-ssh-key directly or configure admin_credentials_key_vault_id plus admin_ssh_key_secret_name."
    }
  }
}
