...
- shared-tag model through `common_tags`, `rg_tags`, and module-generated tags
...
- When `disksize > 0`, Terraform also orders the localization extension after the VM data-disk attachment resource so guest-side disk initialization scripts can see the attached disk.
...
- The VM resource injects the SSH public key supplied through `azure-ssh-key`.
...
- When `app_vm_number > 1`, the module spreads VMs across availability zones by default in round-robin order unless `enable_zone_spread` is disabled.
