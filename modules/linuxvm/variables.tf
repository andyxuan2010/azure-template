variable "common_tags" {
  ...
  validation {
    condition     = alltrue([for k, v in var.common_tags : trimspace(k) != "" && trimspace(v) != ""])
    error_message = "common_tags must contain only non-empty keys and values."
  }
}

#resource group specific tags
# $rgName="rg-ba-eus-prd-shared-management"; $tags=az group show --name $rgName --query tags -o json | ConvertFrom-Json; $keys=$tags.PSObject.Properties.Name | Sort-Object; $maxLen=($keys | % { $_.Length } | measure -Maximum).Maximum; "linux_vm_rg_tags = {"; foreach($key in $keys){$padding=" " * ($maxLen-$key.Length); $value=[string]$tags.$key; '  "{0}"{1} = "{2}"' -f $key,$padding,$value}; "}"
variable "rg_tags" {
  type        = map(string)
  description = "Module-specific tags merged with common_tags and applied to all resources."
  ...
  validation {
    condition     = alltrue([for k, v in var.rg_tags : trimspace(k) != "" && trimspace(v) != ""])
    error_message = "rg_tags must contain only non-empty keys and values."
  }
}
...
variable "azure-user" {
  type        = string
  default     = "azureadmin"
  description = "Admin username for the Linux VM local administrator account. Used directly when disable_password_authentication is false and no Key Vault username fallback is needed."
}

variable "azure-password" {
  type        = string
  default     = ""
  description = "Admin password for the Linux VM local administrator account."
  sensitive   = true
}
...
variable "azure-ssh-key" {
  type        = string
  default     = ""
  description = "SSH public key for the Linux VM local administrator account."
  sensitive   = true
}
...
variable "disable_password_authentication" {
  type        = bool
  default     = false
  description = "Whether to disable password authentication on the Linux VM and enforce SSH key authentication. When true, azure-user and azure-password inputs and their Key Vault fallbacks are ignored."
}
...
variable "admin_username_secret_name" {
  type        = string
  default     = ""
  description = "Optional Key Vault secret name containing the Linux VM admin username. Used only when disable_password_authentication is false and azure-user is empty."
}

variable "admin_password_secret_name" {
  type        = string
  default     = ""
  description = "Optional Key Vault secret name containing the Linux VM admin password. Used only when disable_password_authentication is false and azure-password is empty."
}

variable "admin_ssh_key_secret_name" {
  type        = string
  default     = ""
  description = "Optional Key Vault secret name containing the Linux VM admin SSH public key. Used when azure-ssh-key is empty. The effective SSH key remains mandatory."
}
...
variable "disksize" {
  ...
  validation {
    condition     = var.disksize >= 0
    error_message = "disksize must be zero or a positive number."
  }
}
...
# Common VM sizes for app_vm_size:
...
variable "app_snet" {
  type        = string
  default     = ""
  description = "Legacy subnet name input retained for compatibility. Prefer app_snet_id."
}
variable "app_snet_id" {
  type        = string
  description = "Subnet resource ID used directly for the VM NICs."

  validation {
    condition     = can(regex("^/subscriptions/[^/]+/resourceGroups/[^/]+/providers/Microsoft\\.Network/virtualNetworks/[^/]+/subnets/[^/]+$", trimspace(var.app_snet_id)))
    error_message = "app_snet_id must be a valid Azure subnet resource ID."
  }
}
variable "app_vnet_rg" {
  type        = string
  default     = ""
  description = "Legacy virtual network resource group input retained for compatibility. Prefer app_vnet_id."
}
variable "app_vnet" {
  type        = string
  default     = ""
  description = "Legacy virtual network name input retained for compatibility. Prefer app_vnet_id."
}
variable "app_vnet_id" {
  ...
  validation {
    condition     = trimspace(var.app_vnet_id) == "" || can(regex("^/subscriptions/[^/]+/resourceGroups/[^/]+/providers/Microsoft\\.Network/virtualNetworks/[^/]+$", trimspace(var.app_vnet_id)))
    error_message = "app_vnet_id must be a valid Azure virtual network resource ID when provided."
  }
}
variable "app_vm" {
  type        = string
  ...
  validation {
    condition     = can(regex("^[a-zA-Z0-9-]{1,13}$", trimspace(var.app_vm)))
    error_message = "app_vm must be 1-13 characters using letters, numbers, or hyphens."
  }
}
# variable "app_env" {
#   type = string
...
variable "domain_join_user" {
  ...
  description = "Domain join user in domain\\username format."
