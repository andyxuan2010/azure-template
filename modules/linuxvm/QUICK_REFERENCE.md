...
- `vnet_name`: `string`, required when `vnet_id` is empty
- `vnet_id`: `string`, optional virtual network resource ID override
- `vnet_resource_group_name`: `string`, required when `vnet_id` is empty
- `app_admin_group`: `list(string)`
- `app_user_group`: `list(string)`
- `bastion_resource_group_name`: `string`, defaults to `rg-ba-cc-prod-hub-network`
...
- `inherit_resource_group_tags`: `bool`, defaults to `false`
- `enable_domain_join`: `bool`, defaults to `false`
- `domain_join_user`: `string`, defaults to `""`; when empty, fall back to the configured Key Vault domain join username secret
...
- `private_ip_addresses`: `list(string)`, defaults to `[]`; set one IPv4 address per VM for static private IPs, or leave empty for dynamic allocation
- `enable_zone_spread`: `bool`, defaults to `true`; when enabled and `vm_count > 1`, spreads VMs across availability zones in round-robin order
...
- `public_ssh_source_address_prefixes`: trusted IP/CIDR list required when public networking is enabled
- `tags`: `map(string)`
