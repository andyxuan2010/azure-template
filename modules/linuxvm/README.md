...
- When an extra data disk is enabled through `disksize`, Terraform also waits for the VM data-disk attachment before the localization extension runs.
...
module "linuxvm" {
  ...
  # app_vnet_id = "/subscriptions/<subscription-id>/resourceGroups/<resource-group>/providers/Microsoft.Network/virtualNetworks/<vnet-name>"
  iac_kv = "<iac_kv>"
  iac_rg = "<iac_rg>"
  iac_st = "<iac_st>"
  ...
  azure-ssh-key  = var.linux_vm_admin_ssh_public_key
}
```

## Key Inputs

- `app_rg`: Target resource group name for the Linux VM resources. `string` (required)
- `app_snet`: Existing subnet name used for the VM NICs. `string` (required)
- `app_vm`: Base Linux VM name. Environment suffixes are appended by the module. `string` (required)
- `app_vnet`: Existing virtual network name used for the VM NICs when `app_vnet_id` is not provided. `string` (required when `app_vnet_id` is empty)
- `app_vnet_rg`: Resource group containing the target virtual network when `app_vnet_id` is not provided. `string` (required when `app_vnet_id` is empty)
- `app_vnet_id`: Optional virtual network resource ID override used to resolve the subnet without a separate VNet lookup. `string` (default: `""`)
- `azure-user`: Admin username for the Linux VM local administrator account. `string` (required)
- `azure-password`: Admin password for the Linux VM local administrator account. `string` (required, sensitive)
- `azure-ssh-key`: SSH public key for the Linux VM local administrator account. `string` (required, sensitive)
...
- `enable_zone_spread`: Whether to spread multi-VM deployments across availability zones by default. When enabled and `app_vm_number > 1`, the module assigns zones in round-robin order from `availability_zones`. `bool` (default: `true`)
- `availability_zones`: Availability zones used for round-robin placement when `enable_zone_spread = true` and `app_vm_number > 1`. `list(string)` (default: `["1", "2", "3"]`)
