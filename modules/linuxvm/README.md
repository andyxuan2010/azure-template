...
- Implements resource-group Reader and optional Bastion Reader/Network Contributor assignments for access groups.
- Public SSH requires explicit trusted source IP/CIDR prefixes; internet-wide SSH is not generated.
- Extra managed disks deny network access by default.
- Includes Terraform test coverage files: `tests/live.tftest.hcl`.

Terraform tests use a mocked AzureRM provider and perform plan-only validation.
...
Domain join credential precedence:

- direct module input `domain_join_user` wins when set
- otherwise the module reads `domain_join_username_secret_name` from the shared IaC Key Vault
- the domain join password is read from the `domain-join-password` Key Vault secret
...
- `bastion_resource_group_name`: Resource group containing the Bastion host referenced by `bastion_resource_name`. `string` (default: `rg-ba-cc-prod-hub-network`)
...
- `private_ip_addresses`: Optional static private IP addresses for VM NICs. Leave empty for dynamic allocation; when set, provide exactly one IPv4 address per VM in `vm_count` order. `list(string)` (default: `[]`)
...
- `domain_join_user`: Optional domain join username override. Leave empty to use the configured Key Vault username secret. `string` (default: `""`)
