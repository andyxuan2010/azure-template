...
Explicit `tags` override `inherited_resource_group_tags`. Supplying inherited tags avoids an extra resource-group lookup.
...
For identity-authenticated Capture, use `storage_authentication_type = "SystemAssigned"` with a system identity, or `"UserAssigned"` with `storage_authentication_id` referencing an identity attached to the namespace.
