...
run "plan_named_namespace_secure_baseline" {
  ...
  assert {
    condition     = output.tags.module == "eventhub" && output.tags.ManagedBy == "Terraform" && output.tags.Environment == "Production"
    error_message = "Effective tags did not include standardized Event Hubs tags."
  }
}
...
run "plan_generated_name_without_random" {
  ...
  assert {
    condition     = output.name == "evhns-shared-poc-eus-001"
...
run "plan_private_endpoint_network_rules_cmk_and_dr" {
  ...
  variables {
    ...
    disaster_recovery_config = {
      ...
      partner_namespace_id = "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/rg-ba-eus-prd-shared-management/providers/Microsoft.EventHub/namespaces/evhns-iactest-prod-sec2"
    }
  }
