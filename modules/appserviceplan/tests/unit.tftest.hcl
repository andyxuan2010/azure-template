...
run "plan" {
  ...
  assert {
    condition     = !contains(keys(output.tags), "ManagedBy") && !contains(keys(output.tags), "module") && !contains(keys(output.tags), "name") && !contains(keys(output.tags), "app_env")
    error_message = "App Service Plan tags should not include module-generated ManagedBy, module, name, or app_env tags."
  }
...
run "plan_zone_balanced_premium" {
  ...
  variables {
    ...
    location            = "canadacentral"
    os_type             = "Linux"
    # Premium-only feature coverage; default and autoscale mock tests use B1.
    sku_name                        = "P1v4"
    worker_count                    = 3
