variable "name" {
  description = "Optional App Service Plan name override. Leave empty to generate one from the naming convention."
  type        = string
  default     = ""

  validation {
    condition     = trimspace(var.name) == "" || can(regex("^[a-zA-Z0-9-]{1,40}$", trimspace(var.name)))
    error_message = "name must be empty or between 1 and 40 characters long and can only contain alphanumeric characters and hyphens."
  }
}
