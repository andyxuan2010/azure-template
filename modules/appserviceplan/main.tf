...
# Description: Deploys Azure App Service Plans as the compute hosting layer for App Service and Function App workloads.
#              The module supports standardized naming, SKU and OS selection, zone balancing, autoscale, diagnostics, App Service Environment integration, role assignments, and resource group tag inheritance for governance alignment.
...
#
# Change History:
# - 2026-03-06 v1.0.0: Established reusable appserviceplan module baseline for the CCOE Azure IaC template library.
...
data "azurerm_resource_group" "this" {
  count = trimspace(var.location) == "" || (var.inherit_resource_group_tags && var.inherited_resource_group_tags == null) ? 1 : 0
...
resource "azurerm_monitor_diagnostic_setting" "app_service_plan" {
  count                      = var.enable_diagnostics ? 1 : 0
  name                       = "${local.app_service_plan_name}-diagnostic-setting"
  ...
  # App Service Plans currently expose metrics only, so Azure does not persist a log destination type.
