...
Set an explicit globally unique namespace name for predictable environments. When `name` is empty, the module generates `sb` followed by a random suffix. See the repository [naming convention](../../docs/60-security-governance/naming-convention.md) and [tagging standard](../../docs/60-security-governance/tagging-standard.md).
