locals {
  ...
  generated_name    = trimsuffix(substr("${var.name_prefix}-${local.workload_code}-${local.location_code}-${var.app_env}-${trimspace(var.instance)}", 0, 60), "-")

  static_web_app_name = trimspace(var.name) != "" ? trimspace(var.name) : local.generated_name
