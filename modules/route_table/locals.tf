locals {
  ...
  generated_name   = substr("rt-${local.workload_code}-${local.location_code}-${var.app_env}-${trimspace(var.instance)}", 0, 80)
  route_table_name = trimspace(var.name) != "" ? trimspace(var.name) : local.generated_name
