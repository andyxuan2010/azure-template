variable "name" {
  type        = string
  description = "Optional Azure Container Registry name override. Leave empty to generate one from the naming convention."
  default     = ""
...
variable "network_rule_ip_rules" {
  type        = list(string)
  description = "Optional list of allowed public IPv4 addresses or CIDR ranges for ACR network rules."
  default     = []
  ...
  validation {
    ...
    error_message = "network_rule_ip_rules entries must be valid IPv4 addresses or CIDR ranges."
  }
}
...
variable "instance" {
  ...
  validation {
    ...
    error_message = "instance must be 1-12 lowercase alphanumeric characters for ACR naming."
  }
}
