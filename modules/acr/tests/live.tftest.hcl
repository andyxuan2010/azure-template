...
run "plan" {
  ...
  assert {
    condition     = output.name == "acrccacriactestprod001prod001"
...
run "plan_bare_ipv4_network_rule" {
  ...
  assert {
    condition     = output.name == "acrccipv4registrytest801"
    error_message = "ACR should accept and normalize a bare IPv4 network rule."
  }
}
