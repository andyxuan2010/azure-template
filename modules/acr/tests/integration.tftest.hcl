...
run "plan" {
  ...
  assert {
    condition     = output.name == "acriactestprod001"
    error_message = "ACR explicit name override was not preserved."
  }

  assert {
    condition     = !contains(keys(output.tags), "module") && !contains(keys(output.tags), "ManagedBy") && !contains(keys(output.tags), "name")
    error_message = "ACR tags should not include module-generated ManagedBy, module, or name tags."
  }
...
run "plan_bare_ipv4_network_rule" {
  command = plan
  ...
  assert {
    condition     = output.name == "ipv4registry"
    error_message = "ACR should preserve the explicit name while accepting and normalizing a bare IPv4 network rule."
  }
}

run "reject_invalid_network_rule" {
  command = plan
  ...
  variables {
    network_rule_ip_rules = ["999.999.999.999"]
  }

  expect_failures = [
    var.network_rule_ip_rules,
  ]
}

run "reject_ipv6_network_rule" {
  command = plan
  ...
  variables {
    network_rule_ip_rules = ["2001:db8::1"]
  }
