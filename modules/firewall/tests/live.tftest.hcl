...
run "plan_named_vnet_firewall_baseline" {
  ...
  assert {
    condition     = output.tags.module == "firewall" && output.tags.ManagedBy == "Terraform" && output.tags.Environment == "Production"
    error_message = "Effective tags did not include standardized firewall tags."
  }
}
...
run "plan_generated_name_without_random" {
  ...
  assert {
    condition     = output.name == "afw-hub-poc-eus-001"
