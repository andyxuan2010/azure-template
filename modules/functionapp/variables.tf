...
variable "connection_strings" {
  ...
  validation {
    condition     = length(distinct([for connection in var.connection_strings : lower(trimspace(connection.name))])) == length(var.connection_strings)
    error_message = "connection_strings names must be unique."
  }
}
...
variable "ip_restrictions" {
  ...
  validation {
    condition = alltrue([
      ...
      length(compact([
        try(trimspace(restriction.ip_address), ""),
        try(trimspace(restriction.service_tag), ""),
        try(trimspace(restriction.virtual_network_subnet_id), "")
      ])) == 1
    ])
    error_message = "Each ip_restrictions item must use Allow or Deny and set exactly one of ip_address, service_tag, or virtual_network_subnet_id."
  }

  validation {
    condition = alltrue([
      for restriction in var.ip_restrictions :
      try(trimspace(restriction.ip_address), "") == "" || can(cidrhost(restriction.ip_address, 0))
      ]) && alltrue([
      for restriction in var.ip_restrictions :
      try(trimspace(restriction.virtual_network_subnet_id), "") == "" ||
      can(regex("^/subscriptions/.+/resourceGroups/.+/providers/Microsoft\\.Network/virtualNetworks/.+/subnets/.+$", restriction.virtual_network_subnet_id))
    ])
    error_message = "ip_restrictions IP addresses must be CIDR ranges and subnet IDs must be valid Azure subnet resource IDs."
  }
}
variable "scm_ip_restrictions" {
  ...
  validation {
    ...
    error_message = "Each scm_ip_restrictions item must use Allow or Deny and set exactly one of ip_address, service_tag, or virtual_network_subnet_id."
  }
  ...
  validation {
    ...
    error_message = "scm_ip_restrictions IP addresses must be CIDR ranges and subnet IDs must be valid Azure subnet resource IDs."
  }
}
...
variable "private_endpoint_ip_configurations" {
  ...
  validation {
    condition = alltrue([
      ...
      !strcontains(trimspace(config.private_ip_address), ":") &&
      can(cidrhost("${trimspace(config.private_ip_address)}/32", 0))
    ])
    error_message = "private_endpoint_ip_configurations require non-empty names and valid IPv4 addresses."
  }
...
check "functionapp_input_consistency" {
  assert {
    condition     = !(var.storage_uses_managed_identity && try(trimspace(var.storage_key_vault_secret_id), "") != "")
    error_message = "storage_uses_managed_identity and storage_key_vault_secret_id are mutually exclusive."
  }
  ...
  assert {
    ...
    error_message = "Set storage_account_name unless storage_key_vault_secret_id is provided."
  }

  assert {
    condition = !var.storage_uses_managed_identity || (
      var.system_assigned_identity_enabled || length(local.identity_ids) > 0
    )
    error_message = "storage_uses_managed_identity requires a system-assigned or user-assigned identity."
  }

  assert {
    condition = try(trimspace(var.storage_key_vault_secret_id), "") == "" || (
      var.system_assigned_identity_enabled || length(local.identity_ids) > 0
    )
    error_message = "storage_key_vault_secret_id requires a system-assigned or user-assigned identity."
  }

  assert {
    condition     = !(var.auth_settings != null && var.auth_settings_v2 != null)
    error_message = "Configure auth_settings or auth_settings_v2, but not both."
  }
  ...
  assert {
    ...
    error_message = "enable_diagnostics requires at least one diagnostic destination."
  }
  ...
  assert {
    condition = !var.container_registry_use_managed_identity || (
      try(var.application_stack.docker, null) != null &&
      (var.system_assigned_identity_enabled || length(local.identity_ids) > 0)
    )
    error_message = "container_registry_use_managed_identity requires a Docker application stack and a managed identity."
  }
}
