...
variable "subnets" {
  type = map(object({
    ...
    create_network_security_group_association     = optional(bool, false)
    route_table_id                                = optional(string, "")
    create_route_table_association                = optional(bool, false)
...
variable "app_admin_group" {
  ...
  validation {
    condition     = alltrue([for value in coalesce(var.app_admin_group, []) : trimspace(value) != ""])
...
variable "app_user_group" {
  ...
  validation {
    condition     = alltrue([for value in coalesce(var.app_user_group, []) : trimspace(value) != ""])
