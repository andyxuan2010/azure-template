...
Provisions a Linux or Windows Azure Web App on an existing App Service Plan with hardened site settings, managed identity, authentication, networking, private connectivity, monitoring, and deployment integrations.

## Features

- Supports Linux and Windows Web Apps, including language and container stacks.
- Configures TLS, FTPS, publishing credentials, health checks, logs, IP restrictions, CORS, backups, auto-heal, and sticky settings.
- Supports system-assigned and user-assigned managed identities.
- Integrates with an existing VNet subnet and optionally creates a private endpoint with private DNS.
- Supports App Service built-in Microsoft Entra authentication.
- Creates optional Application Insights and Azure Monitor diagnostics.
- Supports custom hostnames and managed certificates, storage mounts, and Azure Repos Deployment Center.
- Assigns Contributor and Reader access to Microsoft Entra groups.

## Resources Created

The module creates exactly one Linux or Windows Web App. Depending on inputs, it can also create:

- Application Insights;
- custom hostname, managed certificate, and certificate binding resources;
- an Azure Monitor diagnostic setting;
- an App Service source-control resource;
- Microsoft Entra group role assignments;
- a private endpoint and private DNS zone group.

The App Service Plan, resource group, networking, DNS, storage, and monitoring destinations are existing dependencies.

## Prerequisites and Dependencies

- An existing resource group and App Service Plan whose operating system matches `kind`.
- Optional delegated subnet for regional VNet integration.
- Optional private endpoint subnet and `privatelink.azurewebsites.net` private DNS zone.
- Optional Log Analytics workspace, Storage Account, Event Hub, and Application Insights workspace.
- Optional Entra application for built-in authentication.
- Optional storage account and share or container for mounts.

Compose the normal dependency chain as `appserviceplan -> appservice`. See the repository [module dependency guide](../../docs/20-repository/module-usage-and-dependencies.md).

## Provider Configuration

Provider configuration belongs in the calling root:
...
AzureAD is used only for group lookups. The Terraform identity needs permissions for the Web App and every optional resource it enables.
...
- [`examples/active-directory-auth`](examples/active-directory-auth/)
- [`examples/container-app`](examples/container-app/)
- [`examples/storage-mount`](examples/storage-mount/)
- [`examples/windows-app`](examples/windows-app/)

## Important Behavior and Secure Defaults

- HTTPS-only, TLS 1.2 minimum, disabled FTPS, and denied unmatched IP restrictions are module defaults.
- Public network access is enabled by default. Production private applications should disable it only after private endpoint and DNS dependencies are ready.
- Basic publishing credentials should remain disabled; prefer workload identity or Deployment Center authentication.
- `kind` must match the App Service Plan operating system.
- Application settings, connection strings, authentication secrets, storage keys, and site credentials can be stored in Terraform state. Protect the backend and prefer Key Vault references.
- Custom hostname and certificate changes depend on external DNS verification and can be lifecycle-sensitive.
- Diagnostic category discovery reads Azure metadata and can make planning depend on provider access.

## Networking and Private Connectivity

Regional VNet integration and private endpoints solve different flows:

- VNet integration controls outbound access from the app and requires a subnet delegated to `Microsoft.Web/serverFarms`.
- A private endpoint controls inbound access and requires a separate private endpoint subnet plus `privatelink.azurewebsites.net` DNS.

Plan subnet capacity, routing, firewall rules, DNS, and App Service regional support before enabling either feature. Direct subnet and private DNS IDs are preferred over name-based lookups.

## Identity and RBAC

Enable system-assigned identity for normal Azure resource access, or attach user-assigned identities when lifecycle must be independent of the app. The module does not grant those identities access to downstream services.

The optional admin and user group inputs create Contributor and Reader assignments at the Web App scope. Prefer immutable group object IDs and least-privilege custom roles when built-in roles are broader than required.

## Naming and Tagging

Set `name` explicitly or use the module's workload, environment, and instance naming inputs. Custom hostname resources follow caller-provided domain names.

Caller tags override inherited resource-group tags. Follow the repository [naming convention](../../docs/60-security-governance/naming-convention.md) and [tagging standard](../../docs/60-security-governance/tagging-standard.md).
...
The test setup module is also evaluated with mocks; no App Service Plan or Web App is deployed.

## Known Limitations

- The module consumes but does not create an App Service Plan.
- Deployment Center currently models Azure Repos, not every App Service source-control provider.
- Private endpoint DNS linkage requires an existing zone or successful name-based lookup.
- Managed certificates require public DNS validation and are not appropriate for every private-only hostname.
- Some App Service settings and supported stacks vary by OS, SKU, region, and AzureRM provider version.
...
| <a name="input_app_name"></a> [app\_name](#input\_app\_name) | Deprecated alias for name. Use name for the Web App resource name override. | `string` | `""` | no |
...
| <a name="input_inherit_resource_group_tags"></a> [inherit\_resource\_group\_tags](#input\_inherit\_resource\_group\_tags) | Whether to merge tags from the target resource group into App Service resources. | `bool` | `true` | no |
...
| <a name="input_instance"></a> [instance](#input\_instance) | Instance identifier used when app\_name is not provided. | `string` | `"001"` | no |
...
| <a name="input_name"></a> [name](#input\_name) | Optional Web App name override. Leave empty to generate one from the naming convention. | `string` | `""` | no |
