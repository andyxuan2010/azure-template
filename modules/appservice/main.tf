...
#
# Module: appservice
# Description: Deploys Azure App Service web applications for Windows, Linux, or container-based application hosting.
#              The module supports managed identity, authentication settings, application settings, deployment center configuration, Application Insights, diagnostics, private endpoint and VNet integration patterns, and resource group tag inheritance for governance alignment.
...
#
# Change History:
# - 2026-04-17 v1.0.0: Established reusable appservice module baseline for the CCOE Azure IaC template library.
...
resource "azurerm_monitor_diagnostic_setting" "this" {
  ...
  # Azure currently normalizes this Web App diagnostic setting property to null.
...
resource "azurerm_app_service_source_control" "deployment_center" {
  count = var.deployment_center_enabled && local.deployment_center_repo_url != null ? 1 : 0

  app_id   = local.web_app.id
  repo_url = local.deployment_center_repo_url
...
resource "azurerm_private_endpoint" "sites" {
  ...
  name                = "pep-${local.app_name}-sites"
  location            = local.location
  resource_group_name = var.resource_group_name
  subnet_id           = local.private_endpoint_subnet_id_resolved

  private_service_connection {
    name                           = "plsc-${local.app_name}-sites"
