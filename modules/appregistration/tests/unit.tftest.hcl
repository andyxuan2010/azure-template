...
run "plan_federation_without_service_principal" {
  command = plan

  variables {
    display_name             = "appreg-federated-prod"
  ...
  assert {
    condition     = contains(keys(output.federated_identity_credential_ids), "github_main")
    error_message = "Federated identity credentials should not require a service principal."
  }
}
