locals {
  ...
  display_name  = trimspace(var.name) != "" ? trimspace(var.name) : (trimspace(var.display_name) != "" ? trimspace(var.display_name) : substr("appreg-${local.workload_code}-${var.app_env}-${trimspace(var.instance)}", 0, 120))
