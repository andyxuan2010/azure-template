...
variable "app_sqlmi_db" {
  ...
  type        = string
  validation {
    condition     = can(regex("^[a-zA-Z0-9_-]{1,128}$", var.app_sqlmi_db))
...
variable "tags" {
  type        = map(string)
  description = "Custom tags applied to the SQL Managed Database. These override SQL MI tags, and workload is always set from var.workload."
  default     = {}
