locals {
  ...
  containerapp_environment_id = "${local.resource_group_id}/providers/Microsoft.App/managedEnvironments/cae-${var.workload}-${local.region_code}-${var.environment}-001"
  ...
  feature_flags = {
    ...
    enable_availabilityset                 = lookup(var.features, "enable_availabilityset", var.module_plan_enabled.availabilityset)
    ...
    enable_containerapp                    = lookup(var.features, "enable_containerapp", var.module_plan_enabled.containerapp)
