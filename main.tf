...
module "linuxvm" {
  ...
  app_rg                       = local.resource_group_name
  app_snet                     = local.app_subnet_name
  app_snet_id                  = local.app_subnet_id
  app_vnet_rg                  = local.network_resource_group
  app_vnet                     = local.vnet_name
  app_vm                       = local.vm_name
  app_env                      = var.environment
  ...
  azure-ssh-key                = var.azure-ssh-key
...
module "sqldb" {
  ...
  sql_admin_password         = "ChangeMe12345!"
  sql_ad_admin               = "sql-admin-group"
  sql_ad_admin_id            = var.sample_principal_object_id
  sql_sku_name               = "S0"
  sql_rg_name                = local.resource_group_name
  app_env                    = var.environment
  location                   = var.location
