...
write_overrides_file() {
...

  {
    ...
    echo "  },"
    if [[ "${target_module}" == "linuxvm" || "${target_module}" == "winvm" ]]; then
      echo '  "azure-password": "Mock-Password-Only-123!",'
    else
      echo '  "azure-password": "",'
    fi
    if [[ "${target_module}" == "sqlmi" || "${target_module}" == "sqlmi_db" ]]; then
      echo '  "sqlmi_administrator_login_password": "Mock-Sql-Password-Only-123!",'
    else
      echo '  "sqlmi_administrator_login_password": "",'
    fi
    if [[ "${target_module}" == "fortigate" ]]; then
      echo '  "fortigate_admin_ssh_public_key": "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIK6CeDp5G4Z+b0ocYVn9dOidyafLythOOPrO4Qql8ule terraform-module-validation"'
    else
      echo '  "fortigate_admin_ssh_public_key": ""'
    fi
    echo "}"
  } > "${output_file}"
}
